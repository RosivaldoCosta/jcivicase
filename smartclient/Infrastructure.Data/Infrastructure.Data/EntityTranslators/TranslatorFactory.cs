...
{
    public class TranslatorFactory<T,K> : Sante.EMR.SmartClient.Infrastructure.Library.EntityTranslators.EntityMapperTranslator<T, K> where T : new() where K : new()
    {

        public override bool CanTranslate(Type targetType, Type sourceType)
        {
            return true; // base.CanTranslate(targetType, sourceType);
        }

        protected override K BusinessToService(Sante.EMR.SmartClient.Infrastructure.Interface.Services.IEntityTranslatorService service, T value)
        {
            K f = new K();
        ...
        /// <returns></returns>
        protected override T ServiceToBusiness(Sante.EMR.SmartClient.Infrastructure.Interface.Services.IEntityTranslatorService service, K value)
        {
            T form = new T();
