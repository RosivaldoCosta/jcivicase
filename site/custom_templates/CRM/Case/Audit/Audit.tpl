...
*}
<script src="{$config->resourceBase}audit.js" type="text/javascript"></script>
<link rel="stylesheet" type="text/css" href="{$config->resourceBase}style.css" />
...
<tr class="activity{if $smarty.foreach.activityloop.first} selected{/if}" id="civicase-audit-activity-{$smarty.foreach.activityloop.iteration}">
	<td class="indicator">
		...
		<img src="{$config->resourceBase}spacer.gif" width="20" height="20">
		{else}
		<a href="javascript:selectActivity({$smarty.foreach.activityloop.iteration})">
		<img src="{$config->resourceBase}incomplete.gif" width="20" height="20" alt="{ts}Incomplete{/ts}" title="{ts}Incomplete{/ts}">
		</a>
		{/if}
	</td>
	<td>
