<link rel="stylesheet" href="{$rootURL}administrator/templates/system/css/pagePDF.css" type="text/css" />
{include file="CRM/Case/Form/PDF/header.tpl"}
<div class='cdu_print_main'>

	{* Custom Data Top User form*}
	{foreach from=$userinfo_groupTree item=cd_edit key=group_id}
	
		<div class='cdu_print_title4'>{$cd_edit.fields.0.0.label}: {$cd_edit.fields.0.0.value}</div>
		
		<div class='cdu_print_innerTable'>

			<div class='test3_1_1'><div>
			{foreach from=$cd_edit.fields.1 item=element}
				<span>{$element.label}:</span> {$element.value} <br />
			{/foreach}
			</div></div>			
			
			<div class='test3_1_2'><div>
			{foreach from=$cd_edit.fields.2 item=element}
				<span>{$element.label}:</span> {$element.value} <br />
			{/foreach}
			</div></div>
			
			<div class='test3_1_3'><div>
			{foreach from=$cd_edit.fields.3 item=element key=key}
				<span>{$element.label}:</span> {$element.value} <br />
			{/foreach}
			</div></div>
	...
	{/foreach}
    {if $pdf_template}
        {include file="CRM/Case/Form/$pdf_template.tpl"}
    {else}
    ...
    {/if}
		{foreach from=$grTree item=block}
            ...
            {elseif $block.name == 'Caller_Information'}
					...
					<div class='test11_2'>
						...
						<span>{$block.fields.1266.label}:</span>  
							...
							{/foreach} 
						<br />
						<span>{$block.fields.1267.label}:</span> {$block.fields.1267.element_value} <br />
					</div>
			...
			{elseif $block.name == 'Insurance'}
				<div class='insurance_block'>
					...
					<div class='test44'>
					{section name=num loop=$block.fields.121.element_value start=6 max=4}
					    <img src='{$rootURL}images/{$block.fields.121.element_value[num].value}.png'> {$block.fields.121.element_value[num].text}<br />
					{/section}
					</div>
				</div>
			...
			{elseif $block.name == 'Emergency_Contact_If_Client_Is_Under_18'}
				...
				{/if}
			{elseif $block.name == 'Client_Information_Field'}
