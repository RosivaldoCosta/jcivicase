...
var currentID =  "{$latestRevisionID}";
</script>
{if $revs}
<div id="activity-list">
  ...
  <table style="width: 95%; border: 1px solid #CCCCCC;">
      ...
      <tr id='hl_{$row.id}'>
         ...
         {if $changeLast}<td><a href="javascript:setCurrentRevision( {$row.id}, {$latestRevisionID} );" title="{ts}Set this revision as current.{/ts}">{if $row.id != $latestRevisionID}Set as Current{/if}</a></td>{/if}
      </tr>
       {/foreach}
  </table>
</div>
...
{literal}
<script type="text/javascript">
cj('#hl_'+currentID).css('font-weight', 'bold');
//alert(currentID); 
...
function setCurrentRevision( activityId, lastActID ) {
    var cid= {/literal}"{$contactID}"{literal};
   	var viewUrl = {/literal}"{crmURL p='civicrm/case/activity/view' h=0 q="snippet=4" }"{literal}; 
   	cj("#activity-content").load( viewUrl + "&cid=" + cid + "&aid=" + activityId + "&laid=" + lastActID + "&revs=1&setcur=1" );
   	flexReloads = true;
}
</script>
{/literal}
