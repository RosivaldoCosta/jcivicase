...
<table class="caseSelector">
  ...
  <tr id='rowid{$list}{$row.case_id}'>
    ...
    {if $context != 'case'}	
        ...
        <span id="{$list}{$row.case_id}_show">
            <a href="#" onclick="show('caseDetails{$list}{$row.case_id}', 'table-row'); 
                                 ...
                                 return false;"><img src="{$config->resourceBase}TreePlus.gif" class="action-icon" alt="{ts}open section{/ts}"/></a>
        </span>
        <span id="minus{$list}{$row.case_id}_hide">
            <a href="#" onclick="hide('caseDetails{$list}{$row.case_id}'); 
                                 ...
                                 return false;"><img src="{$config->resourceBase}TreeMinus.gif" class="action-icon" alt="{ts}open section{/ts}"/></a>
        </td>
    {/if}	
	...
	<td>
		...
		<span class="description">{ts}Case ID{/ts}: {$row.case_id}</span>
	</td>
    	<td>
		<a href="{crmURL p='civicrm/contact/view' q="reset=1&cid=`$row.contact_id`"}" title="{ts}view contact details{/ts}">{$row.sort_name}</a>{if $row.phone}<br />
		<span class="description">{$row.phone}</span>{/if}<br />
	</td>
    {/if}
	...
	{$row.case_scheduled_activity_type}{else}---{/if}</td>
    <td>{if $row.case_scheduled_activity_date}
        {$row.case_scheduled_activity_date|crmDate}{else}---{/if}</td>
    <td>{$row.case_opened_date|crmDate}</td>
