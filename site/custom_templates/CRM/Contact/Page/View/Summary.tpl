...
{else}
        ...
        <div title="Summary" id="contact-summary" class="ui-tabs-panel ui-widget-content ui-corner-bottom">
            {if $hookContentPlacement neq 3}
                <div class="buttons ui-corner-all">
                    ...
                    <span id="icons">
                        <a title="{ts}vCard record for this contact.{/ts}" href='{crmURL p='civicrm/contact/view/vcard' q="reset=1&cid=$contactId"}'> <img src="{$config->resourceBase}vcard-icon.png" alt="vCard record for this contact." /></a>
                        <a title="{ts}Printer-friendly view of this page.{/ts}" href='{crmURL p='civicrm/contact/view/print' q="reset=1&print=1&cid=$contactId"}'"> <img src="{$config->resourceBase}print-icon.png" alt="Printer-friendly view of this page." /></a>
                    </span>
                </div><!-- .buttons -->
                                ...
                                <table>
                                    {foreach from=$phone item=item}
                                        ...
                                        <tr>
                                            ...
                                            <td>
						<a href="{crmURL p='civicrm/contact/call' q="reset=1&action=makecall&to=$item.phone&cid=$contactId"}" class="edit button" title="{ts}Call{/ts}">
                        				<span><div class="icon edit-icon"></div>{ts}Call{/ts}</span>
                        			</a>
					    </td>
                                        </tr>
                                        {/if}
	...
	cj( function() {
        ...
        cj("#mainTabContainer").tabs( {selected: tabIndex, ajaxOptions: { cache: false }} );
    });
    {/literal}
    </script>
{/if}
