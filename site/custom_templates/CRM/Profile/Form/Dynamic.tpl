...
<script type="text/javascript" src="{$config->resourceBase}js/Common-mini.js"></script>
{/if}
{if ! empty( $fields )}
...
//                   cj("#contact-read").show( );
                   cj("#contact-read").html(response.sortName + cj("#contact-read").html());
                   var dataUrl = {/literal}"{crmURL p='civicrm/ajax/contactlist' q='context=contactinfo' h=0 }{literal}" +'&cid=' + response.contactID;
                  	  cj.ajax({
                  	            url     : dataUrl,
                  	            dataType: "json",
