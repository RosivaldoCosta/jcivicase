{if ! empty( $fields )}
    ...
    {if $groupId }
        <div id="id_{$groupId}_show" class="section-hidden section-hidden-border">
            <a href="#" onclick="hide('id_{$groupId}_show'); show('id_{$groupId}'); return false;"><img src="{$config->resourceBase}TreePlus.gif" class="action-icon" alt="{ts}open section{/ts}"/></a><label>{ts}Edit Search Criteria{/ts}</label><br />
        </div>

        <div id="id_{$groupId}">
            <fieldset><legend><a href="#" onclick="hide('id_{$groupId}'); show('id_{$groupId}_show'); return false;"><img src="{$config->resourceBase}TreeMinus.gif" class="action-icon" alt="{ts}close section{/ts}"/></a>{ts}Search Criteria{/ts}</legend>
    {else}
        <div>
    {/if}
