<div id="help">
    ...
    <fieldset><legend>{ts}Upload Directories{/ts}</legend>
        <table class="form-layout">
            ...
            <tr>  
                <td class="label">{$form.customTemplateFileExtension.label}</td>
                <td>{$form.customTemplateFileExtension.html|crmReplace:class:'huge40'}<br />
                    <span class="description">{ts}Template extension for custom files that should be used before using other regular template files.{/ts}</span>
                </td>    
            </tr>
        </table>
    </fieldset>
