...
{capture assign=plusImg}<img src="{$config->resourceBase}TreePlus.gif" alt="{ts}plus sign{/ts}" style="vertical-align: bottom; height: 20px; width: 20px;" />{/capture}
        ...
        {foreach from=$group.fields item=panelItem  key=panelName name=groupLoop}
            <tr class="{cycle values="odd-row,even-row" name=$groupName}">
                <td style="vertical-align: top; width:24px;">
                    <a href="{$panelItem.url}"{if $panelItem.extra} {$panelItem.extra}{/if} ><img src="{$config->resourceBase}
