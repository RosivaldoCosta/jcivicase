...
if( framework != 'Joomla') {
	jQuery('body').prepend( jQuery("#menu-container").html() );

	//Track Scrolling
	jQuery(window).scroll( function () { 
	   var scroll = document.documentElement.scrollTop || document.body.scrollTop;
	   jQuery('#civicrm-menu').css({top: "scroll", position: "fixed", top: "0px"}); 
	   jQuery('div.sticky-header').css({ 'top' : "23px", position: "fixed" });
	});
} else {
