...
{if $action EQ 4 AND $currentAttachmentURL} {* For View action we exclude the form fields and just show any current attachments. *}
    ...
    <tr>
        ...
        <td class="view-value"><strong><a href="{$currentAttachmentURL}"><img src="/pgcrs/media/civicrm/custom/{$attachmentName}"/></a></strong></td>
    </tr>
    </table>
    </fieldset>
...
{elseif $action NEQ 4}
    ...
    <div id="attachments_show" class="section-hidden section-hidden-border">
      <a href="#" onclick="hide('attachments_show'); show('attachments'); return false;"><img src="{$config->resourceBase}TreePlus.gif" border="0" class="action-icon" alt="open section"/></a><label>{$attachTitle}</label><br />
    </div>
    {/if}
    <div id="attachments" class="section-shown">
    <fieldset {if $noexpand}style="width:92%"{/if}>
    {if !$noexpand}<legend><a href="#" onclick="hide('attachments'); show('attachments_show'); return false;"><img border="0" src="{$config->resourceBase}TreeMinus.gif" class="action-icon" alt="close section"/></a>{$attachTitle}</legend>
    {/if}
        {if $context EQ 'pcpCampaign'}
