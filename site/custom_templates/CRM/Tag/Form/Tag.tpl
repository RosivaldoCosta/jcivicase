...
<script type="text/javascript" src="{$config->resourceBase}rest.js"></script>
<style>
...
contactID={$contactId};
var image = '<img alt="Hide" src="{$config->resourceBase}close.png" />';
{literal}
function hideStatus( ) {
    cj( '#restmsg' ).hide( );
}
...
<fieldset><legend>{ts}Tags{/ts}</legend>
    ...
    <ul id="tagtree" class="tree">
		...
		{if $myType NEQ 'Manager' OR $id EQ 2}       		
        <li id="tag_{$id}">
