...
class CRM_Utils_Hook {
   ...
   static function json_encode(&$json, $params, $selectorElements = null, $context, $id )
   {
        $config =& CRM_Core_Config::singleton( );
        require_once( str_replace( '_', DIRECTORY_SEPARATOR, $config->userHookClass ) . '.php' );
        return
                eval( 'return ' . $config->userHookClass . '::invoke(5,$json, $params, $selectorElements, $context, $id, \'civicrm_json_encode\');');
   }
    /** 
    ...
    static function caseSearchTasks( $objectType, &$tasks ) {
        $config =& CRM_Core_Config::singleton( );
        require_once( str_replace( '_', DIRECTORY_SEPARATOR, $config->userHookClass ) . '.php' );
        $null =& CRM_Core_DAO::$_nullObject;
        ...
        return   
            eval( 'return ' .
                  $config->userHookClass .
                  '::invoke( 2, $objectType, $tasks, $null, $null, $null, \'civicrm_caseSearchTasks\' );' );
    }
    ...
    static function caseHistory( &$activities ) {
    	$config =& CRM_Core_Config::singleton( );
        require_once( str_replace( '_', DIRECTORY_SEPARATOR, $config->userHookClass ) . '.php' );
        $null =& CRM_Core_DAO::$_nullObject;

        return   
            eval( 'return ' .  $config->userHookClass .  '::invoke( 1, $activities, $null, $null, $null, $null, \'civicrm_caseHistory\' );' );
    }
