...
class CRM_Utils_Weight {
    ...
    static function addOrder( &$rows, $daoName, $idName, $returnURL, $filter = null ) {
        ...
        $imageURL  = $config->userFrameworkResourceURL; // . '/arrow';
    ...
    static function fixOrder( ) {
        ...
        if ($daoName == 'CRM_Core_DAO_CustomField')
        {
            $group = explode('=', $filter);
            foreach ($group as $k => $v)
            {
                if ('custom_group_id' == trim($v))
                {
                    $group_id = $group[$k+1];
                    break;
                }
            ...
            $logMessage = 'Option Group "' . CRM_Core_BAO_CustomGroup::getTitle($group_id) . '" was re-ordered';
        }
        else if ($daoName == 'CRM_Core_DAO_CustomField')
        {
            $group_id = 0;
            $logMessage = 'Option Groups were re-ordered';
        }
        else
        {
            $group_id = 0;
            $logMessage = 'Something were re-ordered';
        }
