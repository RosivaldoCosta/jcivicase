...
*/        
        ...
        if ( CRM_Utils_Array::value( 'case_start_date', $query->_returnProperties) ) {
            $query->_select['case_opened_date']  = "civicrm_case.start_date as case_opened_date";
            $query->_element['case_opened_date'] = 1;
            $query->_tables['civicrm_case']     = 1;
        }
    ...
    static function whereClauseSingle( &$values, &$query ) 
    {
		...
		foreach($value as $index => $v)
		{
			...
			$case_status[] = $statuses[$v];
			//echo "!".$cnt.'<br/>';

		}// CRM_Contact_BAO_Query::buildClause( "civicrm_case.status_id", 'IN', $value, 'String' );

            	$query->_where[$grouping][] = "civicrm_case.status_id IN ( " .$query_values . ')';
            	//$value = $statuses[$value];
                $query->_qill[$grouping ][] = ts( 'Case Status %1', array( 1 => $op))  . ' ' . implode( ' ' . ts('AND') . ' ', $case_status );
	    } else {
            $query->_where[$grouping][] = CRM_Contact_BAO_Query::buildClause( "civicrm_case.status_id", $op, $value, 'Int' ); 
                //$query->_where[$grouping][] = "civicrm_case.status_id <> 2"; //", $op, $value, 'Int' );
                //$query->_where[$grouping][] = "civicrm_case.status_id <> 15"; //", $op, $value, 'Int' );
            $value = $statuses[$value];
            $query->_qill[$grouping ][] = ts( 'Case Status %2 %1', array( 1 => $value, 2 => $op) );
	    }
            //echo CRM_Contact_BAO_Query::buildClause( "civicrm_case.status_id", $op, $value, 'Int' );
            //$query->_where[$grouping][] = CRM_Contact_BAO_Query::buildClause( "civicrm_case.status_id", $op, $value, 'Int' );
            $query->_tables['civicrm_case'] = $query->_whereTables['civicrm_case'] = 1;
            return;
		...
		// Hide closed and recommended for closure cases
//            $query->_where[$grouping][] = "(civicrm_case.status_id <> 2)";
//            $query->_where[$grouping][] = "(civicrm_case.status_id <> 15)";
        ...
        case 'case_start_date':
            $query->_where[$grouping][] = CRM_Contact_BAO_Query::buildClause( "civicrm_case.start_date", $op, $value, 'Date' );
            $query->_tables['civicrm_case'] = $query->_whereTables['civicrm_case'] = 1;
            return;
        case 'case_end_date':
            $query->_where[$grouping][] = CRM_Contact_BAO_Query::buildClause( "civicrm_case.end_date", $op, $value, 'Date' );
            $query->_tables['civicrm_case'] = $query->_whereTables['civicrm_case'] = 1;
            return;
        }
    }
...
case_relation_type.id = case_relationship.relationship_type_id )";
        ...
        case 'case_activity':
            $from .= " $side JOIN civicrm_case_activity ON civicrm_case_activity.case_id = civicrm_case.id ";
			$from .= " $side JOIN civicrm_activity case_activity ON ( civicrm_case_activity.activity_id = case_activity.id
...
//                                'case_type_id'                =>      1,
	...
	$form->addElement('text', 'case_id', ts('Case Number'), CRM_Core_DAO::getAttribute('CRM_Case_DAO_Case', 'case_id') );
        ...
        $form->add('select', 'case_status_id',  ts( 'Case Status' ),  
                   array( '' => ts( '- any status -' ) ) + $statuses, false, array('id'=>'case_status_id','multiple'=>'multiple','title'=>ts('Select Status')) );
	//print_r($statuses);
        foreach ( $statuses as $id => $name) {
            $form->addElement('checkbox', "$id",$name);
        }
