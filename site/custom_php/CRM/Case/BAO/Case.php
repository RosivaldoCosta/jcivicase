...
";
	...
	$params = new JParameter( $plugin->params );
        ...
        if ( $type == 'upcoming'  || $type == 'whiteboard') {
...
//                       AND civicrm_activity.activity_date_time < ca2.activity_date_time )";
	    ...
	    $query .= " WHERE ca2.id IS NULL AND civicrm_case.is_deleted=0";
	}
	else
	{
	    $query .= " WHERE civicrm_activity.id IS NULL AND civicrm_case.is_deleted=0";
	}
        ...
        if ( $type == 'upcoming'  || $type == 'whiteboard') {
            $query .=" ORDER BY case_scheduled_activity_date, case_id ASC ";
        } else if ( $type == 'recent' ) {
            $query .= " ORDER BY case_recent_activity_date ASC ";
        }

	if( $type === 'recent')
	    $query = self::getAllCases();
...
/*    static function getCaseActivity( $caseID, &$params, $contactID, $context = null, $userID = null, $type = null )
	...
	$lockstatus_params = new JParameter( $plugin->params );
        ...
        $select = "SELECT count(ca.id) as ismultiple, ca.id as id, civicrm_case.id as case_id,
                        civicrm_case.status_id,
			  ...
			  DATE_ADD(IF(ca.original_id is NULL, cl.modified_date, al.modified_date), INTERVAL 1 DAY) as edit_end_date_time,
			  IF( ca.status_id=2, cl.modified_date,  
       				IF( ca.status_id=3, cl.modified_date, 
				  IF( ca.status_id=4, cl.modified_date, 
				   IF( ca.status_id=5, cl.modified_date, null)))) as completed_date, 
			  IF( ca.status_id=2, completed_c.sort_name,
				IF( ca.status_id=3, completed_c.sort_name,
				  IF( ca.status_id=4, completed_c.sort_name,
					IF( ca.status_id=5, completed_c.sort_name, NULL)))) as completed_contact,
		  	  IF(ca.original_id is NULL, cl.modified_date, al.modified_date) as assigned_date  ";

	if(count($plugin) > 0)
	{
	    $select .= ',IF(cls.locked_'.$lockstatus_params->get('lockstatus_field') .' is null, clo.locked_'.$lockstatus_params->get('lockstatus_field').',cls.locked_'.$lockstatus_params->get('lockstatus_field').') as locked ';
	    $select .= ',IF(cls.entity_id is null, clo.entity_id,cls.entity_id) as lockedstatus_id ';
        ...
        $from  = 'FROM civicrm_case_activity cca 
	   	  ...
	   	  LEFT JOIN civicrm_log cl ON cl.entity_id=ca.id and cl.entity_table = \'civicrm_activity\'
		  LEFT JOIN civicrm_log al ON al.entity_id=ca.original_id and al.entity_table = \'civicrm_activity\' 
		  LEFT JOIN civicrm_contact completed_c ON completed_c.id = cl.modified_id
		  LEFT JOIN civicrm_case on civicrm_case.id = cca.case_id ';

	if(count($plugin) > 0)
	{
	    //$from .= 'LEFT JOIN civicrm_value_lock_status_'.$lockstatus_params->get('lockstatus_table') .' cls ON (cls.activity_id_'.$lockstatus_params->get('activity_id_field') . '=ca.original_id or cls.activity_id_'.$lockstatus_params->get('activity_id_field') .'=ca.id) ';
	    $from .= 'LEFT JOIN civicrm_value_lock_status_'.$lockstatus_params->get('lockstatus_table') .' cls ON (cls.activity_id_'.$lockstatus_params->get('activity_id_field') . '=ca.id) ';
	    $from .= 'LEFT JOIN civicrm_value_lock_status_'.$lockstatus_params->get('lockstatus_table') .' clo ON (clo.activity_id_'.$lockstatus_params->get('activity_id_field').'=ca.original_id) ';
	}
	...
	if(count($plugin) > 0)
	{
	    $where .= ' AND ca.activity_type_id <> '.$lockstatus_params->get('lockstatus_activity_type');
	    //$where .= ' AND (cls.activity_id_'.$lockstatus_params->get('activity_id_field').'=ca.id  OR clo.activity_id_'.$lockstatus_params->get('activity_id_field') .'=ca.original_id )';
	}
	    ...
	    {
		case "OPS":
		    $params['activity_type_id'] = '(13,35,36)';
		    break;
		case "MCT":
		    $params['activity_type_id'] = '(37,35,38,39,40,84)';
		    break;
		case "IFIT":
		    $params['activity_type_id'] = '(50,37,51,39,40)';
		    break;
		case "UCC":
		    $params['activity_type_id'] = '(90,102,82,50,85,40,84,93,98)';
		    break;
		case "CISM":
		    $params['activity_type_id'] = '(91)';
		    break;
		case "Vouchers":
		    $params['activity_type_id'] = '(89,88,86,87)';
		    break;
		case "Tasks":
		    $params['activity_type_id'] = '(34,56,67,68,72,73,74,75,76,77,47,44,45,80,42,78)';
		    break;
		case "Phone_Contacts":
		    $params['activity_type_id'] = '(48)';
		    break;
		case "Status_Change":
		    $params['activity_type_id'] = '(16)';
		    break;
		default:
		    $is_single = 1;
	    ...
	    {
		$where .= " AND ca.activity_type_id IN ".$params['activity_type_id'];
	    }
	}
	...
	if ( CRM_Utils_Array::value( 'activity_date_assigned_low', $params ) )
	{
	    $assignedActivityDateLow = CRM_Utils_Type::escape( CRM_Utils_Date::processDate( $params['activity_date_assigned_low'] ), 'Date' );
	}
	if ( CRM_Utils_Array::value( 'activity_date_assigned_high', $params ) )
	{
	    $assignedActivityDateHigh = CRM_Utils_Type::escape( CRM_Utils_Date::processDate( $params['activity_date_assigned_high'] ), 'Date' );
	}
	if ( CRM_Utils_Array::value( 'activity_date_completed_low', $params ) )
	{
	    $completedActivityDateLow = CRM_Utils_Type::escape( CRM_Utils_Date::processDate( $params['activity_date_completed_low'] ), 'Date' );
	}
	if ( CRM_Utils_Array::value( 'activity_date_completed_high', $params ) )
	{
	    $completedActivityDateHigh = CRM_Utils_Type::escape( CRM_Utils_Date::processDate( $params['activity_date_completed_high'] ), 'Date' );
	}
	...
	if ( !empty( $assignedActivityDateLow ) && !empty( $assignedActivityDateHigh ))
	{
	    $where .= " AND IF(ca.original_id IS NULL, (cl.modified_date >= '{$assignedActivityDateLow}' AND cl.modified_date < '{$assignedActivityDateHigh}'), (al.modified_date >= '{$assignedActivityDateLow}' AND al.modified_date < '{$assignedActivityDateHigh}'))";
	}
	elseif (!empty( $assignedActivityDateLow ))
	{
	    $where .= " AND IF(ca.original_id IS NULL, (cl.modified_date >= '{$assignedActivityDateLow}'), (al.modified_date >= '{$assignedActivityDateLow}'))";
	}
	elseif (!empty( $assignedActivityDateHigh ))
	{
	    $where .= " AND IF(ca.original_id IS NULL, (cl.modified_date < '{$assignedActivityDateHigh}'), (al.modified_date < '{$assignedActivityDateHigh}'))";
	}

	if ( !empty( $completedActivityDateLow ) && !empty( $completedActivityDateHigh ))
	{
	    $where .= " AND ca.status_id=2 AND cl.modified_date >= '{$completedActivityDateLow}' AND cl.modified_date < '{$completedActivityDateHigh}'";
	}
	elseif ( !empty( $completedActivityDateLow ) )
	{
	    $where .= " AND ca.status_id=2 AND cl.modified_date >= '{$completedActivityDateLow}'";
	}
	elseif ( !empty( $completedActivityDateHigh ) )
	{
	    $where .= " AND ca.status_id=2 AND cl.modified_date < '{$completedActivityDateHigh}'";
	}
	...
	//if(isset($_GET['debug']) && $_GET['debug']) echo '<pre>'.print_r($query,true).'</pre>';
	...
	$attachUrl = "{$url}&action=add&atype=96";
	if(count($plugin) > 0)
	    $lockUrl = "{$url}&action=update&atype=".$lockstatus_params->get('lockstatus_activity_type');
	else
	    $lockUrl = "";
...
//        $compStatusNames = array('Completed', 'Left Message', 'Cancelled', 'Unreachable', 'Not Required');
	$compStatusNames = array('Scheduled');
	...
	$signForms = array( //40 => 'Authorization To Release',
		//39 => 'Consent For Services',
		82 => 'Informed Consent',
		85 => 'Human Rights Notification',
		84 => 'Acknowledgememnt for Privacy Practices',
		93 => 'Informed Consent for Medication',
		87 => 'Temporary Housing',
		51 => 'Individual Treatment Plan'
	);

	$flashSignForms = array( 40 => 'Authorization To Release',
		39 => 'Consent For Services');
	$isViewOnlyGroup = self::checkPermissionGroup('View Only');
	$isPsychiatristGroup = self::checkPermissionGroup('Psychiatrist', 'edit');
	    ...
	    $values[$dao->id]['display_date'] = CRM_Utils_Date::customFormat( $dao->display_date, '%m-%E-%Y %h:%i %P' );
	    $values[$dao->id]['status']       = $activityStatus[$dao->status];

	    if( array_key_exists($dao->type,$signForms) || array_key_exists($dao->type,$flashSignForms))
	    {
		$sigprintUrl = CRM_Utils_System::url( "civicrm/case/activity",
			"reset=1&cid={$contactID}&caseid={$caseID}", false, null, false );
		$printUrl = "{$sigprintUrl}&action=update";
	    } else
	    {
		$printUrl = CRM_Utils_System::url( "civicrm/case/activity/view",
			"reset=1&snippet=2&cid={$contactID}&caseid={$dao->case_id}&aid={$dao->id}", false, null, false );
	    }

	    $changeStatus = '';

	    //require_once('CRM/Utils/Hook.php');
	    //CRM_Utils_Hook::casehistory_action($dao);

	    if($dao->type == 16) // change status case

	    {
		$arrStatuses = self::getCaseChangeStatusByActivity($dao->id);
		$changeStatus = ': ' . $arrStatuses['currentStatus'] ;
		if ( $arrStatuses['status'] )
		{
		    $changeStatus .= ' ' . ts('to') . ' ' . $arrStatuses['status'];
		}
	    }
            ...
            if ( $allowView ) {
                $subject = "<a href='javascript:viewActivity( {$dao->id}, {$contactID} );' title='{$viewTitle}'>{$dao->subject}{$changeStatus}</a>";
            }
            $values[$dao->id]['subject'] = $subject;
	    $values[$dao->id]['assigned_date'] = CRM_Utils_Date::customFormat( $dao->assigned_date, '%m-%e-%Y %h:%i %P');
	    $values[$dao->id]['completed_date'] = CRM_Utils_Date::customFormat( $dao->completed_date, '%m-%e-%Y %h:%i %P');
	    $values[$dao->id]['completed_contact'] = $dao->completed_contact;
	    ...
	    $isForm = in_array($dao->type, array_keys(CRM_Core_OptionGroup::values( 'case_forms', false, false,  false, null, 'value' )));
	    if ($dao->status_id == 17)
	    {
		$isWithinTwentyFourHours = true;
		$isForm24 = true;
	    }
	    else
	    {
		$isWithinTwentyFourHours = self::isWithinTwentyFourHourWindow($dao->assigned_date, $dao->edit_end_date_time);
		$isForm24 = self::isTwentyFourHourForm($dao->type);
	    }

	    $isAdminGroup = self::isAdminGroup(); //$session->get( 'userID' ));

		if(isset($_GET['debug']) && $_GET['debug'])
		{
			echo 'setdate: '.$dao->assigned_date.' edit_end_date: '.$dao->edit_end_date_time;
			echo ' iswithin24: '; if($isWithinTwentyFourHours) echo '1'; else echo '0';
			echo ' isform: '.$isForm. ' isform24: '. $isForm24. ' locked: '.$dao->locked.' status: '.$activityStatus[$dao->status].'<br/>';
		}
		...
		if ( !$dao->deleted )
		{
		    if ( (!$isViewOnlyGroup && !$isPsychiatristGroup) || ($isPsychiatristGroup && !$isForm) )
		    {
			$url = "<a href='" .$editUrl.$additionalUrl."'>". ts('Update') . "</a> ";
		    }
		}
	    }
	    else if( $activityStatus[$dao->status] !== 'Completed' && $dao->type != 16)
	    {
		if ( !$dao->deleted )
		{
		    if ( (!$isViewOnlyGroup && !$isPsychiatristGroup) || ($isPsychiatristGroup && !$isForm) )
	    ...
	    else if( $isForm24 && $isWithinTwentyFourHours && !$dao->locked && $dao-type != 16)
	    {
		

		if ($activityStatus[$dao->status] === 'Completed' )
		{
		    if ( !$dao->deleted )
		    {
			if ( (!$isViewOnlyGroup && !$isPsychiatristGroup) || ($isPsychiatristGroup && !$isForm) )
...
//	    /*else if( count($plugin) > 0 && $isForm && !$dao->locked && $dao->type != 16)
//	    {
//		if(isset($_GET['debug']) && $_GET['debug']) echo $dao->type . ' : '.$dao->locked.'<br/>';
//		if ( !$dao->deleted )
//		{
//		    if ( (!$isViewOnlyGroup && !$isPsychiatristGroup) || ($isPsychiatristGroup && !$isForm) )
//		    {
...
//	    }  * /
	    ...
	    if( !$isViewOnlyGroup )
	    {
		if( $isAdminGroup || $isWithinTwentyFourHours )
		{
		    require_once 'CRM/Contact/BAO/Contact.php';
		    $userType = CRM_Contact_BAO_Contact::getContactSubType($_SESSION['CiviCRM']['userID']);

		    if((!$isAdminGroup && $userType != 'Supervisor') && $dao->type == 35)
		    {
	    ...
	    if( array_key_exists($dao->type,$signForms) && !$isViewOnlyGroup)
	    {
		if ( !empty($url) )
		{
		    $url .= " | ";
		}
		$url .= "<a href='" .$attachUrl."&targetId={$dao->id}&sigtype=npapi'>". ts('Sign') . "</a> ";
	    } else if ( array_key_exists($dao->type, $flashSignForms) && !$isViewOnlyGroup)
	    {
		if ( !empty($url) )
		{
		    $url .= " | ";
		}
		$url .= "<a href='" .$attachUrl."&targetId={$dao->id}&sigtype=flash'>". ts('Sign') . "</a> ";
	    ...
	    if($dao->type != 13){
	    	...
	    	$url .= "<a target='new' href='" .$printUrl.$additionalUrl."&snippet=2'>". ts('Print') . "</a> ";
	    }
	    
	    // PDF link for activities
	    if(self::isSetPdfPlugin($dao->type))
	    {
			    if( !empty($url) )
			    {
					$url .= " | ";
			    }
			    $url .= "<a href='" .$editUrl.$additionalUrl."&snippet=3'>". ts('View') . "</a>";
	    }
		...
		if( $isAdminGroup && $isForm)
		{


		    // TODO: Check lock status
		    $name = '';
		    $url .= " | <a  href='" .$lockUrl."&target_activity_id=";

		    if(isset($dao->original_id))
		    {
			$url .= $dao->original_id;
		    } else
		    {
		    ...
		    $url .= "&id=".$dao->lockedstatus_id."'>";

		    if(isset($dao->locked) && $dao->locked)
		    {
			$name = "U";
		    }
		    else if(isset($dao->locked) && !$dao->locked)
		    {
			$name = "L";
		    }
	    ...
	    $task_names = CRM_Core_OptionGroup::values( 'case_tasks', false, false,  false, null, 'label' );
	    //print_r($task_names);
	    foreach( $task_names as $k => $v)
	    {
		//print_r($dao);
		//echo CRM_Core_OptionGroup::getValue('case_tasks',$v, 'name');
		if( $dao->type === CRM_Core_OptionGroup::getValue('case_tasks',$v, 'name'))
		{
		    $values[$dao->id]['class'] = $values[$dao->id]['class']." task";
        }
...
GROUP BY cc.id';
    ...
    /*static function isLocked($activityId)
    {
	locked_1756
    }*/
...
WHERE id IN ('. implode( ',', $copiedActivityIds ) . ')';
    ...
    // Calculate Days Open
    static function calculateDaysOpened($caseID)
    {
	$query = "SELECT TO_DAYS(NOW()) - TO_DAYS(start_date) as days
                  FROM civicrm_case c
                  WHERE c.id = %1"; // {$where} {$groupBy}";
    ...
    // Closed
    static function getDateClosed($caseId)
    {

    }

    // Date Opened
    static function getDateOpened($caseId)
    {

    }

    // Case Type
//    static function getCaseType($caseId)
//    {
//
//    }

    // Referral Source
    static function getReferralSource($contactId)
    {
    ...
    // Risk Factors
    static function getRiskFactors($caseId)
    {

    }

    // Retrieve Intake
    static function getIntake($caseId)
    {
    ...
    /**
     * Function to get Cases Activities on customs conditions
     *
     * @param int    $caseID case id
     * @param array  $params posted params
     * @param int    $contactID contact id
     ...
     * @static
     */
    static function getCasesActivityCustom( $caseID, &$params, $contactID, $context = null )
    {

	$values = array( );

	$select = 'SELECT count(ca.id) as ismultiple, ca.id as id, cca.case_id,
                          ca.activity_type_id as type,
                          ca.activity_type_id as activity_type_id,
                          cc.sort_name as reporter,
                          cc.id as reporter_id,
			  ...
			  DATE_ADD(cl.modified_date, INTERVAL 1 DAY) as edit_end_date_time,
	...
	$from  = 'FROM civicrm_case_activity cca
                  ...
                  LEFT JOIN civicrm_contact acc ON acc.id = caa.assignee_contact_id
	   	  LEFT JOIN civicrm_log cl ON cl.entity_id=ca.id ';

	$where = 'WHERE ca.is_current_revision = 1';
	if(is_numeric($caseID))
	{
	    $where .= ' AND cca.case_id= %1';
	}
	elseif ($caseID === 'all')
	{
	    // all cases
	...
	{
//            $where .= ' AND cca.case_id IN (' . CRM_Core_DAO::escapeString($caseID) . ')';
	    $where .= ' AND cca.case_id IN (' . $caseID . ')';
	}
	$where .= ' AND cl.entity_table="civicrm_activity"';
	/* @todo civicrm_activity ??? */
	if ( CRM_Utils_Array::value( 'reporter_id', $params ) )
	{
	    $where .= " AND ca.source_contact_id = ".CRM_Utils_Type::escape( $params['reporter_id'], 'Integer' );
	}
	...
	if ( CRM_Utils_Array::value( 'activity_type_id', $params ) )
	{
	    ...
	    {
		//            $where .= ' AND cca.case_id IN (' . CRM_Core_DAO::escapeString($caseID) . ')';
		$where .= ' AND ca.activity_type_id IN (' . $activityTypeId . ')';
	    }
...
//			$where .= " AND ca.activity_type_id = ".CRM_Utils_Type::escape( $params['activity_type_id'], 'Integer' );
	...
	$query  = $select . $from . $where . $groupBy . $orderBy;
	$queryCount  = 'SELECT count( * ) ' . $from . $where . $groupBy ;

	if(is_numeric($caseID))
	{
	    $params = array( 1 => array( $caseID, 'Integer' ) );
	    $dao    =& CRM_Core_DAO::executeQuery( $queryCount, $params );
	}
	else
	{
	    $params = array( 1 => array( $caseID, 'String' ) );
	    $dao    =& CRM_Core_DAO::executeQuery( $queryCount);
	}
	$params['total'] = $dao->N;
	...
	$contactViewUrl = CRM_Utils_System::url( "civicrm/contact/view",
		"reset=1&cid=", false, null, false );
	require_once 'CRM/Activity/BAO/ActivityTarget.php';
	$hasViewContact = true; // CRM_Core_Permission::giveMeAllACLs( );
	$viewTitle  = ts('View this activity.');

	while ( $dao->fetch( ) )
	{

	    $clientIds = self::retrieveContactIdsByCaseId( $dao->case_id );
	    $url = CRM_Utils_System::url( "civicrm/case/activity",
		    "reset=1&cid={$contactID}&caseid={$dao->case_id}", false, null, false );
	    $printUrl = CRM_Utils_System::url( "civicrm/case/activity/view",
		    "reset=1&snippet=2&cid={$contactID}&caseid={$dao->case_id}&aid={$dao->id}", false, null, false );
	    $viewUrl    = "{$url}&action=view";
	    $editUrl    = "{$url}&action=update";
	    $deleteUrl  = "{$url}&action=delete";
	    $restoreUrl = "{$url}&action=renew";
	    $caseDeleted = CRM_Core_DAO::getFieldValue( 'CRM_Case_DAO_Case', $dao->case_id, 'is_deleted' );

	    $allowView   = true; // self::checkPermission( $dao->id, 'view',   $dao->activity_type_id, $contactID );
	    $allowEdit   = true; // self::checkPermission( $dao->id, 'edit',   $dao->activity_type_id, $contactID );
	    $allowDelete = true; // self::checkPermission( $dao->id, 'delete', $dao->activity_type_id, $contactID );
	    ...
	    $values[$dao->id]['display_date'] = CRM_Utils_Date::customFormat( $dao->display_date, '%m-%d-%Y %h:%i %P' );
	    $values[$dao->id]['date'] =  CRM_Utils_Date::customFormat($dao->display_date,'%Y%m%d%H%i00');
	    $values[$dao->id]['status']       = $activityStatus[$dao->status];
	    $values[$dao->id]['subject']      = "<a href='{$viewUrl}{$additionalUrl}' title='{$viewTitle}'>{$dao->subject}</a>";
	    ...
	    /*
	        // TODO: Need to add 24 hr requirement for editing cases
	 	// TODO: Need to add ACL requirements
	    $isWithinTwentyFourHours = self::isWithinTwentyFourHourWindow($dao->set_date, $dao->edit_end_date_time);
	    $isAdminGroup = self::isAdminGroup();
...
//                $url .= " | "."<a href='#' onClick='Javascript:fileOnCase( \"move\", \"{$dao->id}\", {$dao->case_id} ); return false;'>". ts('Move To Case') . "</a> ";
//            }
//            if ( self::checkPermission( $dao->id, 'Copy To Case', $dao->activity_type_id ) ) {
//                $url .= " | "."<a href='#' onClick='Javascript:fileOnCase( \"copy\", \"{$dao->id}\", {$dao->case_id} ); return false;'>". ts('Copy To Case') . "</a> ";
//            }


	    $url .= " | <a target='new' href='" .$editUrl.$additionalUrl."&snippet=2'>". ts('Print') . "</a> ";
	    */
    ...
    /**
     * Check special permissions for users concerning groups to which they concern
     *
     * @param $nameGroup
     * @param $operation
     * @param $actTypeId
     * @param $userId
     * @return boolean true if concerning to group
     */
    static function checkPermissionGroup( $nameGroup, $operation = 'edit', $actTypeId = null, $userId = null )
    {
	$allow = false;
	if (!$operation )
	{
	...
	if($operation == 'edit')
	{
	    ...
	    {
		case 'Psychiatrist':
		    if(in_array($nameGroup,$contactGroups))
		    {
			if(!$actTypeId)
			{
			    $allow = true;
			...
			else
			{
			    ...
			    unset($caseTasks[83]); // IHIT Visit
			    unset($caseTasks[43]); // Schedule UCC Appointment
			    if(in_array($actTypeId, array_keys($caseTasks)))
			    {
				$allow = true;
			    }
			}
    ...
    /**
     * Get changed statuses for cases by activity Id
     *
     * @param $activityId
     * @return mixed array
     */
    function getCaseChangeStatusByActivity( $activityId)
    {
	$queryParam = array();

	$query = "SELECT current_case_status_id, case_status_id FROM civicrm_case_change_status WHERE activity_id = {$activityId}";
	...
	$results['currentStatus'] =  $caseStatus[$dao->current_case_status_id] ;
	$results['status'] = $caseStatus[$dao->case_status_id];
	return $results;
    }

     static function isSetPdfPlugin($type) {
        ...
        {
            case 13:
                $pdfplugin = JPluginHelper::getPlugin( 'civicrm', 'intakepdf' );
                return isset($pdfplugin->name);
            case 35:
                $pdfplugin = JPluginHelper::getPlugin( 'civicrm', 'lethalitypdf' );
                return isset($pdfplugin->name);
            case 36:
                $pdfplugin = JPluginHelper::getPlugin( 'civicrm', 'careplanpdf' ); 
                return isset($pdfplugin->name);
            case 37:
                $pdfplugin = JPluginHelper::getPlugin( 'civicrm', 'mctassessmentandtreatmentpdf' );
                return isset($pdfplugin->name);
            case 38:
                $pdfplugin = JPluginHelper::getPlugin( 'civicrm', 'mctdispatchpdf' );
                return isset($pdfplugin->name);
            case 39:
                $pdfplugin = JPluginHelper::getPlugin( 'civicrm', 'consentforservicespdf' );
                return isset($pdfplugin->name);
            case 40:
                $pdfplugin = JPluginHelper::getPlugin( 'civicrm', 'authorizationtoreleasepdf' );
                return isset($pdfplugin->name);
            case 47:
                $pdfplugin = JPluginHelper::getPlugin( 'civicrm', 'dischargesummarypdf' ); // Discharge Summary
                return isset($pdfplugin->name);
            case 51:
                $pdfplugin = JPluginHelper::getPlugin( 'civicrm', 'individualtreatmentplanpdf' );
                return isset($pdfplugin->name);
            case 82:
                $pdfplugin = JPluginHelper::getPlugin( 'civicrm', 'informedconsentpdf' );
                return isset($pdfplugin->name);
            case 84:
                $pdfplugin = JPluginHelper::getPlugin( 'civicrm', 'privacypracticespdf' );
                return isset($pdfplugin->name);
            case 85:
                $pdfplugin = JPluginHelper::getPlugin( 'civicrm', 'humanrightsnotificationpdf' );
                return isset($pdfplugin->name);
            case 87:
                $pdfplugin = JPluginHelper::getPlugin( 'civicrm', 'temporaryhousingpdf' );
                return isset($pdfplugin->name);
            case 90:
                $pdfplugin = JPluginHelper::getPlugin( 'civicrm', 'medicationevaluationpdf' ); // Medication Evaluation
                return isset($pdfplugin->name);
            case 93:
                $pdfplugin = JPluginHelper::getPlugin( 'civicrm', 'consentformedicationpdf' );
                return isset($pdfplugin->name);
            case 101:
                $pdfplugin = JPluginHelper::getPlugin( 'civicrm', 'referralsgivenpdf' );
                return isset($pdfplugin->name);
            case 102:
                $pdfplugin = JPluginHelper::getPlugin( 'civicrm', 'assessmentandtreatmentpdf' );
                return isset($pdfplugin->name);
    ...
    function getCaseClosedBy( $caseId)
    {
	$queryParam = array();
	/*$query = "
                  SELECT ct.display_name,ct.user_id
                  FROM civicrm_activity a
                  INNER JOIN civicrm_case_activity ca ON ( a.activity_type_id=16 AND a.id=ca.activity_id AND a.is_current_revision=1)
                  INNER JOIN civicrm_case c ON ( c.id = ca.case_id AND c.status_id=2 )
                  INNER JOIN civicrm_case_change_status cs ON ( cs.activity_id=a.id AND cs.case_status_id = 2)
		  INNER JOIN civicrm_contact ct ON (ct.id=a.source_contact_id)
                  WHERE c.id = {$caseId}";
	*/
	$query = "SELECT license,auditor FROM view_case_auditors WHERE case_id = {$caseId}";

	$results = array();
	$dao = CRM_Core_DAO::executeQuery($query,$queryParam);
	$dao->fetch();

	$results['name'] = $dao->auditor;
	$results['license'] = $dao->license;
	return $results;
    }

    static function processSignatureCaptureActivity( &$params )
    {
        
	require_once 'CRM/Case/DAO/SignatureCapture.php';
	$signActivityDAO =& new CRM_Case_DAO_SignatureCapture();
	$signActivityDAO->entity_id = $params['activity_id'];
	$signActivityDAO->signature_type_1249 = $params['signature_type'];
	$signActivityDAO->find( true );
	$signActivityDAO->target_id = $params['target_id'];
	$signActivityDAO->signature_data = $params['signature_data'];
    $signActivityDAO->file_name_804 = $params['file_name_804'];
	$signActivityDAO->save();
	//echo '<pre>'.print_r($signActivityDAO,true).'</pre>';exit(0);
    }

    static function processChangeCaseStatusActivity( &$params )
    {
	require_once 'CRM/Case/DAO/ChangeCaseStatus.php';
	$caseActivityDAO =& new CRM_Case_DAO_ChangeCaseStatus();
	$caseActivityDAO->activity_id = $params['activity_id'];
	$caseActivityDAO->case_id = $params['case_id'];
	$caseActivityDAO->case_status_id = $params['case_status_id'];
	$caseActivityDAO->current_case_status_id = $params['current_case_status_id'];

	$caseActivityDAO->find( true );
	$caseActivityDAO->save();
	//echo '<pre>'.print_r($caseActivityDAO,true).'</pre>';exit(0);
	...
	$query = 'SELECT civicrm_case.id as case_id,'.
		'cov_type.label as case_type,'.
		'cov_type.name as case_type_name,'.
		'cov_status.label as case_status,'.
		'cov_status.label as case_status_name,'.
		...
		' LEFT JOIN civicrm_option_group cog_type ON cog_type.name = \'case_type\''.
		' LEFT JOIN civicrm_option_group cog_status ON cog_status.name = \'case_status\''.
		' LEFT JOIN civicrm_option_value cov_status ON ( civicrm_case.status_id = cov_status.value AND cog_status.id = cov_status.option_group_id )'.
		' LEFT JOIN civicrm_option_value cov_type ON ( civicrm_case.case_type_id = cov_type.value AND cog_type.id = cov_type.option_group_id )'.
		' LEFT JOIN civicrm_case_contact ON civicrm_case.id = civicrm_case_contact.case_id'.
		' LEFT JOIN civicrm_contact ON civicrm_case_contact.contact_id = civicrm_contact.id';
    ...
    function getReferralCases($allCases = true, $userID = null, $type = '105')
    {
	$condition = " AND civicrm_case.is_deleted  = 0 AND ((civicrm_case.status_id = 1 and civicrm_contact.id IS NOT NULL) OR (civicrm_case.status_id <> 1 ))";
	...
	*/
	...
	$query .=
		"SELECT count(*) FROM civicrm_case
                  INNER JOIN civicrm_case_activity
                        ON civicrm_case_activity.case_id = civicrm_case.id
	...
	$query .= " INNER JOIN civicrm_activity
                             ON ( civicrm_case_activity.activity_id = civicrm_activity.id
                                  AND civicrm_activity.is_current_revision = 1
                                  AND civicrm_activity.activity_type_id = $type)";
	$query .= "
                  LEFT JOIN civicrm_option_group aog ON aog.name = 'activity_type'
                  LEFT JOIN civicrm_option_value aov
                        ON ( civicrm_activity.activity_type_id = aov.value
                  ...
                  LEFT JOIN civicrm_option_group cog_type ON cog_type.name = 'case_type'
                  LEFT JOIN civicrm_option_value cov_type ON ( civicrm_case.case_type_id = cov_type.value
                             AND cog_type.id = cov_type.option_group_id )

                  LEFT JOIN civicrm_option_group cog_status ON cog_status.name = 'case_status'
                  LEFT JOIN civicrm_option_value cov_status ON ( civicrm_case.status_id = cov_status.value
                            AND cog_status.id = cov_status.option_group_id ) ";
	...
	if ($type == "contact_cases")
	{
	    $query = self::getAllCases();
	    $query .= " WHERE civicrm_contact.id = ".$userID;
	}
	else
	{
	    $query = self::getCaseActivityQuery( $type, $userID, $condition );
	...
	// check whether the user has view/edit signer permission
	$permissions = array( CRM_Core_Permission::VIEW );
    ...
    static function getSignatureData(&$params)
    {
	$result = array();
	$query = "SELECT s.id,s.signature_data,s.target_id,s.signature_type_1249 as type, s.file_name_804";
	$query .= " FROM civicrm_value_signature_capture_fields_144 s";
	$query .= " INNER JOIN civicrm_activity a ON a.id=s.entity_id";
	$query .= " WHERE a.is_deleted=0 AND s.target_id=". $params['target_id'];
	$res = CRM_Core_DAO::executeQuery( $query, CRM_Core_DAO::$_nullArray );
//	echo $query;exit(0);
	while( $res->fetch() )
	{
	    	$result[$res->id]['signature_data'] = $res->signature_data;
        	$result[$res->id]['file_name_804'] = $res->file_name_804;
	    	$result[$res->id]['type'] = $res->type;
	}
...
AND option_group_case_status.id = case_status.option_group_id )
LEFT JOIN civicrm_relationship case_relationship ON ( case_relationship.case_id  = civicrm_case.id )
WHERE civicrm_case.is_deleted =0  AND ((status_id = 1 and civicrm_contact.id IS NOT NULL) OR (status_id <> 1 ))";
//{$myCaseWhereClause} {$myGroupByClause}";
	$res = CRM_Core_DAO::executeQuery( $query, CRM_Core_DAO::$_nullArray );
	while( $res->fetch() )
	{
	    if ( CRM_Utils_Array::value('TotalCases', $rows) &&  CRM_Utils_Array::value($res->case_status, $rows['TotalCases']) )
	    {
		$rows['TotalCases'][$res->case_status]['count'] = $rows['TotalCases'][$res->case_status]['count'] + 1;
	    } else
	    {
		$rows['TotalCases'][$res->case_status] = array( 'count' => 1,
			'url'   => CRM_Utils_System::url( 'civicrm/case/search',
			"reset=1&force=1&status={$res->status_id}&all=1" ) //&type={$res->case_type_id}
		);
	    }
	}
    ...
    static function getCasePhoneActivity( $caseID, &$params, $contactID )
    {
	$values = array( );
	$config =& CRM_Core_Config::singleton( );
	...
	$select = 'SELECT count(ca.id) as ismultiple, ca.id as id,
                        civicrm_case.status_id,
                          ...
                          ca.priority_id as priority,
			  cl.modified_date as set_date,
			  tc.person_contacted_723 as person_contacted,
			  tc.org__724 as org,
			  tc.phone_775 as phone,
			  tc.type_of_call_726 as type,';
	if ($config->customTemplateFileExtension === "escrs")
	    $select .= 'tc.narrative_1258 as subject,';
	else
	    $select .= 'tc.narrative_1427 as subject,';

	$select .= 'ca.activity_type_id as activity_type,
			  DATE_ADD(cl.modified_date, INTERVAL 1 DAY) as edit_end_date_time,
			  IF( ca.status_id=2, cl.modified_date, IF( ca.status_id=3, cl.modified_date, NULL)) as completed_date,
		  	  IF(ca.original_id is NULL, cl.modified_date, al.modified_date) as assigned_date ';

	$from  = 'FROM civicrm_case_activity cca
                  INNER JOIN civicrm_activity ca ON (ca.id = cca.activity_id and ca.activity_type_id=48)
                  INNER JOIN civicrm_contact cc ON cc.id = ca.source_contact_id
                  LEFT OUTER JOIN civicrm_option_group og ON og.name="activity_status"
                  LEFT OUTER JOIN civicrm_option_value ov ON ov.option_group_id=og.id AND ov.name="Scheduled"
		  LEFT JOIN civicrm_value_telephone_contact_138 tc on tc.entity_id=ca.id
                  LEFT JOIN civicrm_activity_assignment caa ON caa.activity_id = ca.id
                  LEFT JOIN civicrm_contact acc ON acc.id = caa.assignee_contact_id
	   	  LEFT JOIN civicrm_log cl ON cl.entity_id=ca.id and cl.entity_table = \'civicrm_activity\'
		  LEFT JOIN civicrm_log al ON al.entity_id=ca.original_id and al.entity_table = \'civicrm_activity\' 
	...
	$where = 'WHERE cca.case_id= %1 AND ca.is_current_revision = 1';
	...
	$orderBy = " ORDER BY display_date DESC";
	...
	//$log = &JLog::getInstance('civicrm.log.php');
	//echo $query;
	...
	*/
	...
	while ( $dao->fetch( ) )
	{
	    ...
	    $values[$dao->id]['display_date'] = CRM_Utils_Date::customFormat( $dao->display_date, '%m-%e-%Y %h:%i %P' );
	    $values[$dao->id]['status']       = $activityStatus[$dao->status];
	    $values[$dao->id]['person_contacted']       = $dao->person_contacted;
	    $values[$dao->id]['org']       = $dao->org;
	    $values[$dao->id]['type']       = $dao->type;
	    $values[$dao->id]['activity_type']       = $dao->activity_type;
	    $values[$dao->id]['phone']       = $dao->phone;
	    $values[$dao->id]['subject']      = str_replace(array("\r\n", "\r", "\n"), ' ', $dao->subject); 
	    $values[$dao->id]['assigned_date'] = CRM_Utils_Date::customFormat( $dao->assigned_date, '%m-%e-%Y %h:%i %P');
	    $values[$dao->id]['completed_date'] = CRM_Utils_Date::customFormat( $dao->completed_date, '%m-%e-%Y %h:%i %P');
	    ...
	    {
		$isWithinTwentyFourHours = self::isWithinTwentyFourHourWindow($dao->set_date, $dao->edit_end_date_time);
		$isForm24 = self::isTwentyFourHourForm($dao->activity_type);
		//echo $dao->activity_type;
	    }

	    $isAdminGroup = self::isAdminGroup(); //$session->get( 'userID' ));

	    if( $isAdminGroup || ($isWithinTwentyFourHours && $isForm24 && $activityStatus[$doa->status] === 'Completed') || ($isPsychiatristGroup && !self::checkPermissionGroup('Psychiatrist', 'edit', $dao->type) ) || !$isForm24 )
	    {
		if ( !$dao->deleted )
		{
		    if ( (!$isViewOnlyGroup && !$isPsychiatristGroup) || ($isPsychiatristGroup && !$isForm) )
		...
		$url .= "<a href='" .$attachUrl."&targetId={$dao->id}&type=npapi'>". ts('Sign') . "</a> ";
	    } else if(array_key_exists($dao->type, $flashSignForms) && !$isViewOnlyGroup)
	    {
		if ( !empty($url) )
		{
		...
		$url .= "<a href='" .$attachUrl."&targetId={$dao->id}&type=flash'>". ts('Sign') . "</a> ";
    ...
    /**
     * Function to determine whether the activity is a task
     *
     * @param int $type
     * @return returns true if activity is a form which is governed by the 24 hr window
     *
     * @static
     */
    static function isTask($type)
    {
	//CRM_Core_OptionGroup::getValue( 'case_tasks', 'Completed', 'name' );
	$activity_types = CRM_Core_OptionGroup::values( 'case_tasks', false, false,  false, null, 'value' );
    ...
    /**
     * Function to determine whether the case activity is a form governed by the 24 hr window
     *
     * @param int $type
     * @return returns true if activity is a form which is governed by the 24 hr window
     *
     * @static
     */
    static function isTwentyFourHourForm($type)
    {
	switch ($type)
	{
	    case 13: //Intake
	    case 17: //Recommend for Closure
	    case 25: //Mental Health Evaluation
	    case 34: // Stat Check - How Are You
	    case 35: //Lethality
	    case 36: //Care Plan
	    case 37: // Assessment and Treatment
	    case 38: //MCT Dispatch
	    case 39: //Consent for Services
	    case 40: // Authorization To Release
	    case 42:
	    case 43: // UCC Appointment
	    case 44: // Dispatch MCT
	    case 45: // Other Task
	    case 47: // Discharge Summary
	    case 48: // Phone Contact
	    case 50:
	    case 56: // Confirm Appointment
	    case 58:  // Send Out PAC Evaluation
	    case 59: // Assess Lethality
	    case 60: // If No Call Back
	    case 61: // Offer More Services
	    case 62: // Relative
	    case 63: // Give Referrals To Alt
	    case 64: // Client Providers
	    case 65:
	    case 67: // Call Referrals
	    case 68: // Stat Check - Assess Lethality
	    case 69: // Stat Check - IHIT Referral
	    case 70: // Sate Check - Schedule UCC Therapist
	    case 71: // Stat Check - Schedule UCC Med Eval
	    case 72: // Stat Check - If no call back
	    case 73: // Stat Check - Offer Services
	    case 74: // Stat Check - Relative
	    case 75: // Stat Check - Give Referrals to Al
	    case 76: // Stat Check - Clients Providers
	    case 77:
	    case 78: // Supervisor to Review
	    case 80: // Attach Document
	    case 82:
	    case 86:
	    case 87:
	    case 88:
	    ...
	    case 99:
	    case 102:
	    case 104: // Call to Schedule UCC Appt
	    case 105: // IFIT Referral
	    default:
		return true;
		break;
	}
    ...
    /**
     * Function to determine whether the case activity is within the 24 hr window
     *
     * @param date $startDate
     * @param date $endDate
     *
     * @return returns true if today's date is within startDate and endDate range
     *
     * @static
     */

    static function isAdminGroup()
    {
	//$session =& CRM_Core_Session::singleton( );
	$me                     = & JFactory::getUser();
	$acl                    =& JFactory::getACL();
	$objectID       = $acl->get_object_id( 'users', $me->get('id'), 'ARO' );

	$groups         = $acl->get_object_groups( $objectID, 'ARO' );
	$this_group = strtolower( $acl->get_group_name( $groups[0], 'ARO' ) );
	if ( ( $this_group == 'super administrator' ) || ( $this_group == 'administrator')) // || ($this_group == 'manager'))
	    return true;
     ...
     * @static
     */
    static function isWithinTwentyFourHourWindow($startDate,$endDate)
    {
	     require_once "CRM/Utils/Date.php";
	//echo CRM_Utils_Date::getRange( $startDate, $endDate  );
	return CRM_Utils_Date::getRange( $startDate, $endDate  );
	...
	foreach($compStatusNames as $name)
	{
	    $compStatusValues[] = CRM_Core_OptionGroup::getValue( 'activity_status', $name, 'label' );
	}
	...
	while ( $dao->fetch( ) )
	{
	    ...
	    $values[$dao->id]['id']           = $dao->id;
	    $values[$dao->id]['type']         = (isset($activityTypes[$dao->type]['label']))?$activityTypes[$dao->type]['label']:'';
	    ...
	    $values[$dao->id]['subject']      = "<a href='javascript:viewActivity( {$dao->id}, {$contactID} );' title='{$viewTitle}'>{$dao->subject}{$changeStatus}</a>";
	    $values[$dao->id]['assigned_date'] = CRM_Utils_Date::customFormat( $dao->assigned_date, '%m-%e-%Y %h:%i %P');
	    $values[$dao->id]['completed_date'] = CRM_Utils_Date::customFormat( $dao->completed_date, '%m-%e-%Y %h:%i %P');
	    $values[$dao->id]['completed_contact'] = $dao->completed_contact;
		...
		if(isset($_GET['debug']) && $_GET['debug'])
		{
			echo ' ' .$dao->id. ' setdate: '.$dao->assigned_date.' edit_end_date: '.$dao->edit_end_date_time;
			echo ' iswithin24: '; if($isWithinTwentyFourHours) echo '1'; else echo '0';
			echo ' isform: '.$isForm. ' isform24: '. $isForm24. ' locked: '.$dao->locked.' status: '.$activityStatus[$dao->status].'<br/>';
		}
	    ...
	    else if( $isForm24 && $isWithinTwentyFourHours && $dao->type != 16)
	    {

		if(isset($_GET['debug']) && $_GET['debug']) echo $isWithinTwentyFourHours . ' ' .$dao->type . ' : '.$dao->locked.'<br/>';
	    ...
	    else if( count($plugin) > 0 && $isForm24 && $isWithinTwentyFourHours && isset($dao->locked) && $dao->locked==0 && $dao-type != 16)
	    {
	    ...
	    else if( count($plugin) > 0 && $isForm && isset($dao->locked) && $dao->locked==0 && $dao->type != 16)
	    {
		if(isset($_GET['debug']) && $_GET['debug']) echo $dao->type . ' : '.$dao->locked.'<br/>';
		...
		if( $isAdminGroup && $isForm)
		{
		    ...
		    if(isset($dao->locked) && $dao->locked==0)
		    {
			$name = "U";
		    }
		    else if(isset($dao->locked) && $dao->locked==1)
		    {
			$name = "L";
		    } else {
			$name = "L";
