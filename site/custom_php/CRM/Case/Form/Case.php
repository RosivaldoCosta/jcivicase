...
*/
            ...
            //$aTypes       = $xmlProcessor->get( 'OperationsCenter', 'ActivityTypes', true );
		$aTypes  = CRM_Core_OptionGroup::values( 'case_tasks', false, false, false); //, $activityCondition );
                //print_r($aTypes);
                //print_r($tasks);
	...
	*/
	...
	$element =& $this->add('select', 'followup_activity_type_id', ts('Followup task'),
                               array( '' => '- '.ts('select activity').' -' ) + $aTypes,
                               true, 
			       array('onchange' => "buildCustomData( 'Activity', this.value );") );


	$this->add('text', 'followup_activity_subject','Subject', CRM_Core_DAO::getAttribute('CRM_Activity_DAO_Activity', 'subject'), true);
	$this->add('text', 'interval','in','size=4' );
	$this->add('select', 'interval_unit',null,CRM_Core_OptionGroup::values('recur_frequency_units',false,false,false,null,'name'), false);
	


        $this->add( 'text', 'activity_subject', ts('Subject'), array('maxlength' => '128'), true);
...
//	$this->addRule('followup_activity_type_id', 'You must select a followup activity', 'required');
    }
	...
	// create follow up activity if needed
        ...
        if ( CRM_Utils_Array::value('followup_activity_type_id', $params) ) {
            $followupActivity = CRM_Activity_BAO_Activity::createFollowupActivity( $this->_activityId, $params );
