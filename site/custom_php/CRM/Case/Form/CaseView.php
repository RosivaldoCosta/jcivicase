...
*/        
	...
	if( count($activities) > 0 )
	{
		...
		{
			if(array_key_exists("Client Information", $report["customGroups"]) && is_array($report["customGroups"]["Client Information"]))
	        		$this->assign( 'clientInformation', $report["customGroups"]["Client Information"] );

			if(array_key_exists("Insurance", $report["customGroups"]))
        			$this->assign( 'insurance', $report["customGroups"]["Insurance"] );

			if(array_key_exists("Financial", $report["customGroups"]))
        			$this->assign( 'financial', $report["customGroups"]["Financial"] );
		}
	})*/
        ...
        $this->assign( 'contactTags', self::getTagsImages($this->_contactID) );
        ...
        $this->_caseDetails = array( 'case_type'       => $caseType,
				     ...
				     'case_closed_date' => $values['case_end_date']
                                   );
	    ...
	    $caseNotEdit = ( $user->get('usertype') == 'Manager') && ($values['case_status_id'] == 15 || $values['case_status_id'] == 2) ; // Recommended for Closure or Closed
        ...
        $this->assign( 'contactGroups', $contactGroups);

    	if(in_array('Interns', $contactGroups))
	{
    		$this->assign( 'isIntern', true);
	}
    	else
    		$this->assign('isIntern', false);

    	if( $user->get('usertype') === 'Manager')
    		$this->assign( 'isManager', true);
    	else
    		$this->assign('isManager', false);
        ...
        // denied permission "Edit" for members of 'View Only' group
        if(!$caseNotEdit && in_array('View Only',$contactGroups))
        {
            $caseNotEdit = true;
        }
	    $this->assign( 'caseNotEdit', $caseNotEdit);
    	...
    	$atArray = array('activity_type_id' => 16);
        /*$changeStatusActivities = CRM_Case_BAO_Case::getCaseActivity( $this->_caseID,
                                                                $atArray,
                                                                $this->_contactID );
	*/
//	print_r($changeStatusActivities);
	...
	// Auditor group have access to audit capability
    	$session = CRM_Core_Session::singleton();
       	$userId = $session->get('userID');
	...
	//print_r($contactGroupList);
        if ( $contactGroupList ) {
                    foreach ( $contactGroupList as $k => $group ) {
			if($group['title'] === 'Auditors')
			{
				$this->assign('isAuditor', 1);
				break;
			}
                    }
         }
...
//        }
        ...
        //////////////////////
        $this->addDate( 'activity_date_assigned_low', ts('Assigned Date From'), false, array( 'formatType' => 'searchDate') );
        $this->addDate( 'activity_date_completed_low', ts('Completed Dates From'), false, array( 'formatType' => 'searchDate') );
        $this->addDate( 'activity_date_assigned_high', ts('To'), false, array( 'formatType' => 'searchDate') );
        $this->addDate( 'activity_date_completed_high', ts('To'), false, array( 'formatType' => 'searchDate') );
        ...
        $aTypesFilter = array(  "OPS" => ts('OPS'),
                                ...
                                "Tasks" => ts('Tasks'),
                                "Phone_Contacts" => ts('Phone Contacts'),
                                "Status_Change" => ts('Status Change')
                                );
	...
	// in order to set the color indicator appropiately
        require_once 'CRM/Core/BAO/EntityTag.php';
        ...
        if ( $entityTag ) {
            ...
            $this->assign('clientTags', $categories );
	    //print_r($categories);
        }

	require_once 'CRM/Contact/BAO/Client.php';
	CRM_Contact_BAO_Client::buildForm_user_info($this);
    ...
    /**
     * Get image for tag by contacts
     *
     * @param integer $entityID - Contact ID
     * @return mixed $tagImages - array by images with path
     */
    public function getTagsImages($entityID)
    {
        require_once 'CRM/Core/BAO/EntityTag.php';
        $tagImages = array();
        ...
        $tags = CRM_Core_BAO_EntityTag::getTag( $entityID);
        if(count($tags))
        {
            $allTag =& CRM_Core_PseudoConstant::tag();
            $tagsMap = CRM_Core_OptionGroup::values('tag_icon_map');
            foreach ($tags as $tag)
            {
                $tagImagesTmp = array();
                if(isset($allTag[$tag]))
                {
                    $tagImagesTmp['name'] = $allTag[$tag];
                    $tagImagesTmp['image'] = array_search(mb_strtolower($allTag[$tag]),$tagsMap);
                    if($tagImagesTmp['image'] !== FALSE)
                    {
                        $tagImages[] = $tagImagesTmp;
                    }
                }
            };
        }
