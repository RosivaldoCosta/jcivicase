...
class CRM_Case_Form_Search extends CRM_Core_Form
{
    ...
    { 
        ...
        $selector =& new CRM_Case_Selector_Search( $this->_queryParams,
                                                   $this->_action,
                                                   ' AND ( case_status_id <> 2 OR case_status_id <> 15 )',
                                                   $this->_single,
                                                   $this->_limit,
                                                   $this->_context );
    ...
    function fixFormValues( )
    {
        ...
        if ( $cid ) {
            ...
            if ( !CRM_Utils_Request::retrieve( 'all', 'Positive', $session ) ) {
                $this->_formValues['case_owner']   = $_SESSION['CiviCRM'][$this->controller->_name]['case_owner'];
                $this->_formValues['case_mycases']   = $_SESSION['CiviCRM'][$this->controller->_name]['case_mycases'];
