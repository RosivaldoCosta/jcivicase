...
class CRM_Case_Form_ActivityView extends CRM_Core_Form
{
    ...
    public function preProcess() 
    {
        ...
        if ( $revs ) {
            ...
            $priorActivities = CRM_Activity_BAO_Activity::getPriorAcitivities( $activityID, false, true );
        
            // if activity is original and selected as latest version id
            if( !$latestRevisionID ){
            	$latestRevisionID = $activityID;
            }
            /* do not display "List all" link if activity has no revisions (only one revision)
             * but display if original id is selected as the latest revision (and there are other revisions that belong to the activity)
             */
            if(count($priorActivities) == 1){
            	$listView = true;
            }
            ...
            //$countPriorActivities = CRM_Activity_BAO_Activity::getPriorCount( $activityID, true );
            $countPriorActivities = count(CRM_Activity_BAO_Activity::getPriorAcitivities( $activityID, false, true ));
