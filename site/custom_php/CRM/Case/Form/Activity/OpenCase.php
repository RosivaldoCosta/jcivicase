...
class CRM_Case_Form_Activity_OpenCase
{
    ...
    static function preProcess( &$form )
    {
        ...
        $form->assign( 'caseType', CRM_Utils_Request::retrieve( 'caseType', 'String', $form ));
    }
   ...
   /**
    ...
    function setDefaultValues( &$form )
    {
        ...
        $caseType = CRM_Utils_Request::retrieve( 'caseType', 'String', $form );
        $depart = CRM_Utils_Request::retrieve( 'depart', 'String', $form );
    if( $caseType === 'mct' || $depart === 'mct')
	{
		$defaults['activity_subject'] = "Open MCT Case";
		$defaults['case_type_id'] = "2";
	}
	else
	{
		$defaults['activity_subject'] = "Open OPS Case";
		$defaults['case_type_id'] = "1";
	}
	...
	$tmp = array();
        ...
        $form->addRule('followup_activity_type_id', ts('Follow Up task is required when creating a new case.'), 'required');
    ...
    public function endPostProcess( &$form, &$params, $activity )
    {
        ...
        if(isset($form->_activityTypeId) && $form->_activityTypeId == 13 && ((isset($params['custom_4_-1']) && $params['custom_4_-1'] == 'Yes')
        || (isset($params['custom_116_-1']) && $params['custom_116_-1'] == 'Yes'))
           )
        {
			$atArray = array('activity_type_id' => 13);
			$activities = CRM_Case_BAO_Case::getCaseActivity( $params['case_id'], $atArray, $params['contact_id'] );
			$activities = array_keys($activities);
			if( count($activities) > 0 )
			{
				$activities = $activities[0];
	        		$params['statusMsg'] .= ts(' It is necessary to fill form  Lethality Rating Sheet since in this form you have specified "Yes"  for "Suicide Ideation" or "Suicide Attempt"');
                		$session->replaceUserContext(CRM_Utils_System::url('civicrm/case/activity', 	// &id=517
                                                                   "reset=1&action=update&cid={$form->_currentlyViewedContactId}&caseid={$params['case_id']}&id={$activities}&newlethality=1") );
			}
        }
        else
        {
	...
	// create follow up activity if needed
        ...
        if ( CRM_Utils_Array::value('followup_activity_type_id', $params) ) {
              ...
              if ( $followupActivity ) {
                       $caseParams = array( 'activity_id' => $followupActivity->id, 'case_id'     => $params['case_id']   );
...
static function getValue($groupTitle, $fieldName, $caseID, $contactID)
        {
                ...
                if( count($activities) > 0 )
                {
                       ...
                       $report       = $xmlProcessor->getActivityInfo( $contactID, $activities, true );
                        ...
                        if(isset($report["customGroups"][$groupTitle]) && is_array($report["customGroups"][$groupTitle]))
                        {
	
				//if( $groupTitle === 'Caller Information' ) print_r($report['customGroups'][$groupTitle]);
                                foreach($report['customGroups'][$groupTitle] as $k => $v)
                                {
					/*if( $groupTitle === 'Caller Information' )
					{
                                    		//if( $v['label'] === $fieldName) return $v['value'];
						echo $v['label'] . ' = ' . $fieldName .'<br/>';
					}*/
                                    if( $v['label'] === $fieldName) return $v['value'];
