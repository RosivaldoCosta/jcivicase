...
";
    ...
    function &getActivityInfo( $clientID, $activityID, $anyActivity = false ) {
        ...
        if ( ! array_key_exists($index, $activityInfos) ) {
            ...
            $query = "
SELECT     a.*, aa.assignee_contact_id as assigneeID, at.target_contact_id as targetID, current_case_status_id, case_status_id
{$selectCaseActivity}
FROM       civicrm_activity a
{$joinCaseActivity}
...
LEFT JOIN civicrm_case_change_status cs ON cs.activity_id=a.id
WHERE      a.id = %1
";
    ...
    function &getActivity( $clientID,
        ...
        if($activityDAO->activity_type_id == 16) // change status case
        {
            require_once 'CRM/Case/BAO/Case.php';
            $arrStatuses = CRM_Case_BAO_Case::getCaseChangeStatusByActivity($activityDAO->id);
...
//
//        	$changeStatus = ': ' . $caseStatus[$activityDAO->current_case_status_id] ;
//        	if ($activityDAO->case_status_id && isset($caseStatus[$activityDAO->case_status_id]))
//        	{
//        		$changeStatus .= ' ' . ts('to') . ' ' . $caseStatus[$activityDAO->case_status_id];
//        	}
        }

        $activity['fields'][] = array( 'label' => 'Subject',
                                       'value' => $this->redact( $activityDAO->subject . $changeStatus ),
                                       'type'  => 'Memo' );
    ...
    function getCustomData( $clientID,
        ...
        foreach ( $sql as $tableName => $sqlClause ) {
            ...
            if ( $dao->fetch( ) ) {
                ...
                {
                    if ($typeValue['fieldID'] == 791)
                    {
                        require_once 'CRM/Case/BAO/Case.php';
                        $atArray = array('activity_type_id' => 101); // Out Referrals Given to Client
                        $activities = CRM_Case_BAO_Case::getCaseActivity( $activityDAO->caseID, $atArray, $clientID );
                        ...
                        {
                            foreach ($refTypeOption[0] as $v)
                            {
                                $key = array_search($v['value'], $dataArr[0]);
                                if ($key !== false)
                                {
                                    $value .=  $v['label'];
                                    foreach ($refTypeOption[1] as $v1)
                                    {
...
WHERE  cf.custom_group_id = cg.id AND cf.is_active = 1 
