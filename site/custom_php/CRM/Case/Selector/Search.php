...
//                                'case_type_id',
	...
	{
		//print_r($a);
		if( !array_key_exists('case_scheduled_activity_date',$a) && !array_key_exists('case_scheduled_activity_date',$b))
			return 0;
		if( !array_key_exists('case_scheduled_activity_date',$a) && array_key_exists('case_scheduled_activity_date',$b))
			return -1;
		if( array_key_exists('case_scheduled_activity_date',$a) && !array_key_exists('case_scheduled_activity_date',$b))
		...
		$date1 = strtotime($a['case_scheduled_activity_date']);
		$date2 = strtotime($b['case_scheduled_activity_date']);
     ...
     */
     function &getRows($action, $offset, $rowCount, $sort, $output = null) 
     {
		...
		if( $sort->getCurrentSortID() == 6 )
		{
			if($sort->getCurrentSortDirection() == 1)
				usort($rows,array($this, "cmpASC"));
			else
				usort($rows,array($this,"cmpDESC"));
             		//$rows = $sort->orderByNextScheduledActivity($rows);
		}
         }
         return $rows;
     }
...
/*                                          array(
					...
					*/
                                          ...
                                          array(
                                                'name'      => ts('Next Task Due Date'),
                                                'sort'      => 'case_scheduled_activity_date',
                                                'direction' => CRM_Utils_Sort::ASCENDING,
                                                ),
                                          array(
                                                'name'      => ts('Case Opened Date'),
                                                'sort'      => 'case_start_date',
                                                'direction' => CRM_Utils_Sort::ASCENDING,
                                                ),
