...
class CRM_Case_Page_DashBoard extends CRM_Core_Page 
{
    ...
    function preProcess( ) 
    {
        ...
        $allCases = true; //CRM_Utils_Request::retrieve( 'all', 'Positive', $session );
        ...
        $summary  = CRM_Case_BAO_Case::getCaseTotals($allCases, $userID ); //getCasesSummary( $allCases, $userID );
        $upcoming = CRM_Case_BAO_Case::getCases( $allCases, $userID, 'whiteboard');

	$referral_type = "105";

        $referralsTotal = CRM_Case_BAO_Case::getReferralCases( $allCases, $userID, $referral_type);
	//print_r($referrals);
        
        $this->assign('casesSummary',  $summary);
        $this->assign('referralsTotal',  $referralsTotal);
