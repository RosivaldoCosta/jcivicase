...
class CRM_Contact_Form_Contact extends CRM_Core_Form
{
    ...
    function setDefaultValues( )
    {
	    ...
	    /* Custom change */
            if ( $this->_contactId ) 
            {
                ...
                $defaults['home_URL'] = $contact->home_url;
            }
	    /* Custom change end */
        ...
        foreach ( $blocks as $name => $label ) {
            ...
            if ( CRM_Utils_Array::value( $name, $fields ) && is_array( $fields[$name] ) ) {
                foreach ( $fields[$name] as $instance => $blockValues ) 
                {
                    ...
                    /* Custom change */
                    if ( $name == 'phone' && CRM_Utils_Array::value( $name, $blockValues ) ) 
                    {
                        $pattern = '/^\(\d{3}\)\d{3}-\d{4}/';
                        if( preg_match($pattern, $blockValues['phone']) == 0)
                        {
                            $errors["{$name}[$instance][phone]"] =
                                ts('Invalid phone format %1. Please enter phone number in correct format: (XXX)XXX-XXXX', array( 1=> $blockValues['phone'] ) );
                        }
                    }
		    /* Custom change end */
                    if ( $dataExists ) {
	...
	/* Custom change */
        $home_URL = CRM_Utils_Array::value( 'home_URL', $params, '' );
        if(!is_string($home_URL) || $home_URL == '')
        {
            $params['home_URL'] = 'http://';
        }
	/* Custom change end */
