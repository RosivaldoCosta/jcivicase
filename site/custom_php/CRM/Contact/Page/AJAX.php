...
class CRM_Contact_Page_AJAX
{
    static function getContactList( &$config )
    {
        ...
        if ($context == 'contactinfo') {
            CRM_Contact_Page_AJAX::getContactInfo( $config );
        }
...
WHERE sort_name LIKE '%$name%'";
    ...
    function enableDisable( &$config ) {
        ...
        if ( isset( $isActive ) ) {
             ...
             if ( $updated ) {
                if (method_exists($recordBAO, 'getTitle'))
                {
                    $rec_title = call_user_func_array(array($recordBAO,'getTitle'), array($recordID));
                    if ($recordBAO == 'CRM_Core_BAO_CustomField')
                    {
                        $logMessage = 'Custom field "' . $rec_title . '"' . $logMessage;
                    }  
                    else if ($recordBAO == 'CRM_Core_BAO_CustomGroup')
                    {
                        $logMessage = 'Custom group ' . $rec_title . $logMessage;
                    }
                }
                else
                {
                    $logMessage = 'Something ' . $logMessage;
                }
                ...
                $logParams = array(
                                   'entity_table'  => 'civicrm_custom_group' ,
                                   'entity_id'     => $recordID,
                                   'modified_id'   => $id,
...
WHERE ce.on_hold = 0 AND cc.is_deceased = 0 AND cc.do_not_email = 0 AND {$queryString} ";
    ...
    /**
     * Get information from profile contacts
     *
     * @param array $config
     */
    static function getContactInfo( &$config )
    {
        require_once 'CRM/Core/BAO/Preferences.php';

        $contact_id   = CRM_Utils_Array::value( 'cid', $_GET );
        $contact_id   = CRM_Utils_Type::escape( $contact_id, 'Positive' );

        $defaults = CRM_Contact_Page_AJAX::getContactInfoProfile($contact_id);
	 ...
	 * @return array
	 */
	public function getContactInfoProfile($contact_id = NULL )
	{
	    $defaults = array();
		if($contact_id)
		{
	    	$params = array();
	    	$params['id'] = $params['contact_id'] = $contact_id;
	    	$defaultsAlt = array();
	    	require_once 'CRM/Contact/BAO/Contact.php';
	    	...
	    	{
	    		// DOB
	    		$birth_date = CRM_Utils_Date::customFormat(CRM_Utils_Date::processDate($client->birth_date),'%m/%d/%Y');
	    		if(!empty($birth_date))
	    		{
		    		$defaults['custom_70_-1'] = $birth_date ;
		    		// Age Bracket
		    		$age = CRM_Utils_Date::calculateAge($client->birth_date);
		    		$year = (!isset($age['years']))?$age['months'] . ' m':$age['years'];
		    		$defaults['custom_71_-1'] = $year;
	    		}
	    		//	Gender
	    		switch ($client->gender_id)
	    		{
	    			case 1:
	    				$defaults['custom_72_-1'] = 'F';
	    				break;
	    			case 2:
	    				$defaults['custom_72_-1'] = 'M';
	    				break;
	    		}
	    		//	Phone
	    		if(isset($$defaultsAlt['phone'][1]['phone']))
	    		{
	    			$defaults['custom_67_-1'] = $defaultsAlt['phone'][1]['phone'];
	    		}
	    		if(isset($defaultsAlt['address'][1]))
	    		{
	    			$defaults['custom_66_-1'] = $defaultsAlt['address'][1]['postal_code'];
	    			$defaults['custom_64_-1'] = $defaultsAlt['address'][1]['city'];
	    			$defaults['custom_65_-1'] = $defaultsAlt['address'][1]['state_province_id'];
	    			$defaults['custom_60_-1'] = $defaultsAlt['address'][1]['street_address'];
                    $defaults['custom_1448_-1'] = $defaultsAlt['address'][1]['street_address'];
	    		}
	            ...
	            $groupTree =& CRM_Core_BAO_CustomGroup::getTree( 'Contact',
	                                                             $form ,
	                                                             $contact_id );
	    		if(isset($groupTree[67]['fields'][280]['customValue'][1]['data']))
	    		{
	    			$defaults['custom_67_-1'] = $groupTree[67]['fields'][280]['customValue'][1]['data'];
	    		}
//                // Race
//	    		if(isset($groupTree[67]['fields'][813]['customValue'][1]['data']))
//	    		{
//	    			$defaults['custom_390_-1'] = $groupTree[67]['fields'][813]['customValue'][1]['data'];
//	    		}
                // Veteran
	    		if(isset($groupTree[67]['fields'][805]['customValue'][1]['data']))
	    		{
	    			$defaults['custom_62_-1'] = $groupTree[67]['fields'][805]['customValue'][1]['data'];
	    		}
//echo '<pre>' . __METHOD__ . ' ' . print_r($groupTree, true) . '</pre>';
            }
		}
		return $defaults;
	}

    static function getContacts( &$config )
    {
        $context = CRM_Utils_Array::value( 'context', $_GET );
        if ($context == 'contactinfo') {
            CRM_Contact_Page_AJAX::getContactInfo( $config );
        ...
        require_once 'CRM/Core/BAO/Preferences.php';
        $name   = CRM_Utils_Array::value( 'query', $_POST );
        $name   = CRM_Utils_Type::escape( $name, 'String' );
        $limit  = '10';
        $list   = array_keys( CRM_Core_BAO_Preferences::valueOptions( 'contact_autocomplete_options' ), '1' );
        $select = array( 'sort_name' );
        ...
        if( $stype = CRM_Utils_Array::value( 'stype', $_GET) ) {
            $where .= " AND contact_sub_type = \"$stype\"";
        }
...
";
	...
	$response = '';
        CRM_Utils_Hook::json_encode( $response,
                                          $dao,
					  null,
                                          CRM_Utils_Array::value( 'context', $_GET ),
                                          CRM_Utils_Array::value( 'id', $_GET ) );
	...
	if( $response === '')
	{
        ...
        //return organization name if doesn't exist in db
        if( ! $contactList && CRM_Utils_Array::value( 'org', $_GET) ) {
            echo CRM_Utils_Array::value( 'query', $_GET );
        }
        exit();
    }
