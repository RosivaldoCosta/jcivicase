...
/**
 * Main page for viewing Care Plans.
 *
 */
class CRM_Contact_Page_View_Plan extends CRM_Contact_Page_View_Note 
{
    /**
    ...
    function edit( ) {
        ...
        $controller =& new CRM_Core_Controller_Simple( 'CRM_Plan_Form_Plan', ts('Care Plan'), $this->_action );
        $controller->setEmbedded( true );

        // set the userContext stack
        $session =& CRM_Core_Session::singleton();
        $url = CRM_Utils_System::url('civicrm/contact/view', 'action=browse&selectedChild=plan&cid=' . $this->_contactId );
        $session->pushUserContext( $url );

        if (CRM_Utils_Request::retrieve('confirmed', 'Boolean',
                                        CRM_Core_DAO::$_nullObject )) {
            CRM_Core_BAO_Plan::del( $this->_id);
        ...
        if (!(self::$_links)) {
            ...
            self::$_links = array(
                                  CRM_Core_Action::VIEW    => array(
                                                                    'name'  => ts('View'),
                                                                    'url'   => 'civicrm/contact/view/plan',
                                                                    'qs'    => 'action=view&reset=1&cid=%%cid%%&id=%%id%%&selectedChild=plan',
                                                                    'title' => ts('View Care Plan')
                                                                    ),
                                  CRM_Core_Action::UPDATE  => array(
                                                                    'name'  => ts('Edit'),
                                                                    'url'   => 'civicrm/contact/view/plan',

                                                                    'qs'    => 'action=update&reset=1&cid=%%cid%%&id=%%id%%&selectedChild=plan',
                                                                    'title' => ts('Edit Plan')
                                                                    ),
                                  CRM_Core_Action::DELETE  => array(
                                                                    'name'  => ts('Delete'),
                                                                    'url'   => 'civicrm/contact/view/plan',
                                                                    'qs'    => 'action=delete&reset=1&cid=%%cid%%&id=%%id%%&selectedChild=plan',
                                                                    'extra' => 'onclick = "if (confirm(\'' . $deleteExtra . '\') ) this.href+=\'&amp;confirmed=1\'; else return false;"',                                                                    
                                                                    'title' => ts('Delete Care Plan')
                                                                    ),
                                  );
        }
        return self::$_links;
