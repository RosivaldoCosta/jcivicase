...
class CRM_Contact_DAO_Contact extends CRM_Core_DAO
{
   ...
   /**
    * returns an array of fields that are different between the two objects and also the old and new value 
    */
   function diff($contact_b)
   {
        $fields = array();
	foreach(self::$_fields as $k=>$v)
	{
			if($k === 'birth_date')
			{
				if( $contact_b->$k !== CRM_Utils_Date::customFormat( $this->$k, '%Y-%m-%d' ))
					$fields[$k] = array('from' => $contact_b->$k, 'to' => $this->$k);	
			} 
			elseif ($k === 'address') 
			{
				echo $contact_b->$k;
			...
			else 
			{
				...
				if($type == 2)
				{
					...
					} elseif( isset($this->$k) && $this->$k != '' && $this->$k !== 'null')
					{
						if( $a !== $b)
							$fields[$k] = array('from' => $contact_b->$k, 'to' => $this->$k);	
					}
