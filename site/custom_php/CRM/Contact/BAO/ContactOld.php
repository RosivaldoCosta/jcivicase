...
class CRM_Contact_BAO_Contact extends CRM_Contact_DAO_Contact 
{
	...
	static function getEmployeeDetailField($contactId, $column)
	{

        $sql = "SELECT ".   $column; 
	$sql .= "FROM     jos_contact_details cd 
		LEFT JOIN jos_contact c ON c.id = cd.user_id
		LEFT JOIN civicrm_contact cc ON cc.user_unique_id = c.id
		WHERE     cc.id = " . CRM_Utils_Type::escape($contactId, 'Integer');
...
WHERE     civicrm_contact.id = " . CRM_Utils_Type::escape($id, 'Integer');
    ...
    function deleteContact( $id )
    {
        ...
        $casesList = CRM_Case_BAO_Case::getCases(true, $id, "contact_cases");
        ...
        //delete contacts cases
        if ($casesList && !empty($casesList))
        {
            foreach ($casesList as $v)
            {
                CRM_Case_BAO_Case::deleteCase( $v['case_id'] , true );
            }
        }
        $contact->delete( );
