...
/**
  ...
  /**
	...
	if(isset($_GET['debug']) && $_GET['debug'])
         	CRM_Core_Error::debug( $this->_customQuery );

	if($this->_customQuery) 
        	$this->_fromClause       = self::fromClause( $this->_tables     , null, null, $this->_primaryLocation, $this->_mode, $this->_customQuery->activityTypes() );
	else 
	...
	//print_r($this->_customQuery->_whereTables); exit;
     ...
     * @static
     */
    static function fromClause( &$tables , $inner = null, $right = null, $primaryLocation = true, $mode = 1, $activityTypes = null ) 
    {
	...
	//print_r($tables);exit;
		...
		/*	
		    ...
		    $from .= " $side JOIN civicrm_activity ON civicrm_activity.id = civicrm_case_activity.activity_id "; // and civicrm_activity.activity_type_id=13 ";
		    if($activityTypes)
		    {
				$activity_types = array();
				for($i=0; $i<=count($activityTypes)-1; $i++)
				{
					$activity_types[$activityTypes[$i]] = $i;
				}

			$activity_keys = array_keys($activity_types);
			$from .= " AND (";
			if(count($activity_keys) > 1)
			{
				for($i=0; $i<=count($activity_keys)-1; $i++)
				{
					if($i<count($activity_keys)-1)
			     			$from .= " civicrm_activity.activity_type_id=".substr($activity_keys[$i],1,strlen($activity_keys[$i])-2). " OR";
					else
			     			$from .= " civicrm_activity.activity_type_id=".substr($activity_keys[$i],1,strlen($activity_keys[$i])-2);
						
				}
			} else {
			     	$from .= "civicrm_activity.activity_type_id=".substr($activity_keys[0],1,strlen($activity_keys[0])-2);
    ...
    function contactType( &$values ) 
    {
        ...
        //if ( ! empty( $clause ) ) { //fix for CRM-771
            ...
            $this->_where[$grouping][] = 'contact_a.contact_type IN ( \'Individual\' )';
            //echo $this->_where[$grouping][0];// = 'contact_a.contact_type IN ( \'Individual\' )';
	
            //$this->_qill [$grouping][]  = ts('Contact Type') . ' - ' . implode( ' ' . ts('or') . ' ', $clause );
            $this->_qill [$grouping][]  = ts('Contact Type') . ' - ' . implode( ' ' . ts('or') . ' ', "\'Individual\'" );
...
";
    ...
    function sortName( &$values ) 
    {
        ...
        $this->_tables['civicrm_case_contact'] = 1; 
        $this->_qill[$grouping][]  = ts( 'Case ID' ) . " $op '$n'";
        if ( empty( $name ) ) {
            return;
        }
