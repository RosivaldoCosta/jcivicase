...
class CRM_Contact_Selector extends CRM_Core_Selector_Base implements CRM_Core_Selector_API 
{
	...
	if(isset($_GET['debug']) && $_GET['debug'])
            CRM_Core_Error::debug( 'f', $this->_ufGroupID);
        ...
        $this->_query   =& new CRM_Contact_BAO_Query( $this->_params,
            ...
            //CRM_Core_Error::debug( 'sql', $this->_query );
        ...
        if (!(self::$_links)) {
            self::$_links = array(
                                  CRM_Core_Action::VIEW   => array(
                                                                   'name'     => ts('View History'),
                                                                   'url'      => 'civicrm/contact/view',
                                                                   'qs'       => 'reset=1&cid=%%id%%#Cases',
                                                                   'title'    => ts('View Case History'),
                                                                   'ref'      => 'view-contact'
                                                                  ),
                                  CRM_Core_Action::ADD => array(
                                                                   'name'     => ts('New Case'),
                                                                   'url'      => 'civicrm/case/add',
                                                                   'qs'       => 'reset=1&action=add&atype=13&cid=%%id%%',
                                                                   'title'    => ts('Open New Case'),
                                                                   'ref'      => 'new-case'
                                                                  )
            ...
            if ( $contextMenu ) {
                ...
                self::$_links[6950]  = array(
                                                                   'name'     => ts('New MCT Case'),
                                                                   'url'      => 'civicrm/case/add',
                                                                   'qs'       => 'reset=1&action=add&caseType=mct&atype=13&cid=%%id%%',
                                                                   'title'    => ts('Open New MCT Case'),
                                                                   'ref'      => 'new-case'
                                                                  );
            }
        }
