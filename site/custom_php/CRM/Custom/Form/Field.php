...
class CRM_Custom_Form_Field extends CRM_Core_Form 
{
    ...
    private static $_dataToHTML = 
        array(
              ...
              array('CaseReference' => 'Select Cases', 'Multi-Select' => 'Multi-Select Case'),
              array('Phone' => 'Phone')
              );
        ...
        if (self::$_dataToLabels == null) {
            self::$_dataToLabels 
                = array(
                        ...
                        array('CaseReference' => ts('Select Cases'), 'Multi-Select' => ts('Multi-Select Case')),
                        array('Phone' => ts('Phone'))
                        );
        }
    }
    ...
    static function formRule( &$fields, &$files, &$self ) 
    {
        ...
        if ( $default ) {
            ...
            case 'Phone':
                if ( ! CRM_Utils_Rule::phone( $default ) ) {
                    $errors['default_value'] = ts( 'Please enter a valid phone number.  Please use format:(еее)еее-ееее.' );
                }
                break;
