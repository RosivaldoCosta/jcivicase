...
class CRM_Report_Form_Case_Activity_Interns extends CRM_Report_Form {
                                ...
                                array( 'dao'     => 'CRM_Activity_DAO_Activity',
                                       ...
                                       array(  'id'                => 
                                               ...
                                               array( 'title'      => ts( 'Activity Status' ),
                                                      ...
                                                      'type'       =>  CRM_Utils_Type::T_STRING ), 
                                               'is_current_revision' => 
                                               array( 'title'      => ts( 'Is Current Version' ),
                                               		  'required'   => true ,
                                                      'default'    => true ,
                                               		  'type'       =>  CRM_Utils_Type::T_STRING),
                                               ),
                                       ...
                                       array( 'activity_date_time'  => 
                                              ...
                                              'is_current_revision' => 
                                              array( 'title'        => ts( 'Activity Is Current' ),
    ...
    function addPhoneContactFieldColumns(&$columns)
    {
        ...
        //CRM_Core_Error::debug('columns', $columns);
        ...
        while($crmDAO->fetch())
        {
        	//CRM_Core_Error::debug('curTable', $curTable);
        	//CRM_Core_Error::debug('table_name', $crmDAO->table_name);
                if ($curTable == '') {
                        $curTable = $crmDAO->table_name;
                        $curExt = $crmDAO->ext;
                } elseif (!array_key_exists($curTable,$columns)) {
	...
	*/
    ...
    function from( ) {
        ...
        /*$this->_from .= " LEFT JOIN civicrm_value_telephone_contact_138 value_telephone_contact_138_civireport
		 ON value_telephone_contact_138_civireport.entity_id = {$this->_aliases['civicrm_activity']}.id ";
	*/
    }
    ...
    function alterDisplay( &$rows ) {
        ...
        foreach ( $rows as $rowNum => $row ) {
            ...
            if ( array_key_exists('civicrm_activity_is_current_revision', $row ) ) {
                if ( $value = $row['civicrm_activity_is_current_revision'] ) {
                    $rows[$rowNum]['civicrm_activity_is_current_revision'] = $activityRev[$value];
                    $entryFound = true;
                } elseif($value == 0){
                    $rows[$rowNum]['civicrm_activity_is_current_revision'] = $activityRev[0];
                    $entryFound = true;
                }
            }
