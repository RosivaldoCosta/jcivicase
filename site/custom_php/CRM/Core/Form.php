...
class CRM_Core_Form extends HTML_QuickForm_Page {
    ...
    function getTemplateFileName() {
        $config =& CRM_Core_Config::singleton( );
        $customTemplateFile = $config->customTemplateDir . str_replace('_',DIRECTORY_SEPARATOR, CRM_Utils_System::getClassName($this) ) .'.tpl.' . $config->customTemplateFileExtension;
            //echo "!!".$customTemplateFile;
        if( file_exists($customTemplateFile) )
        {
                $tplname = str_replace( '_', DIRECTORY_SEPARATOR, CRM_Utils_System::getClassName( $this ) ) .'.tpl.'.$config->customTemplateFileExtension;
        }
        else
        {
