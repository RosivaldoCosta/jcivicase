...
class CRM_Core_Action {
    ...
    static function formLink( &$links, $mask, $values ) {
        ...
        if ( $showDiv ) {
            ...
            $resultDiv = "<div style='position:relative;'><ul id='panel_xx' class='panel'><li>{$resultDiv}</li></ul></div>";
            $resultDivText = ts('more');
        }

        if ($resultDiv) {
        	$result = "<span>{$resultLink}</span><span class='btn-slide' id='xx'>{$resultDivText}{$resultDiv}</span>";
        } else {
