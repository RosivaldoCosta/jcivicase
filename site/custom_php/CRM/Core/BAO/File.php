...
class CRM_Core_BAO_File extends CRM_Core_DAO_File {
    ...
    public function filePostProcess($data, $fileID, 
        ...
        if ( ! rename( $data, $directoryName . DIRECTORY_SEPARATOR . $filename ) ) {
            CRM_Core_Error::fatal( ts( 'Could not move custom file to custom upload directory: ' . $data . ' , ' . $directoryName ) );
    ...
    public function &getEntityFile( $entityTable, $entityID ) {
        ...
        while ( $dao->fetch( ) ) {
            ...
            $result['upload_date'] = $dao->upload_date;
...
AND       CEF.entity_id    = %2";
    ...
    static function buildSignedAttachments( &$form, $entityTable, $entityID = null, $numAttachments = null ) {
	...
	$currentAttachmentURL = array();
		...
		$config =& CRM_Core_Config::singleton( );
            foreach ( $currentAttachments as $fileID => $attach ) {
                if( $attach['mime_type'] === 'image/jpeg')
                {
                    $currentAttachmentURL[$attach['fileName']] = "<img class='signed-attachments' src='".JURI::root(). "/media/civicrm/custom/".$attach['fileName']."' />";

                }
            }
            //return implode( $separator, $currentAttachmentURL );
		$form->assign('embedAttachmentTags',$currentAttachmentURL);
        $form->embedAttachmentTags = $currentAttachmentURL;
        }
    }
    ...
    static function getLastSignedAttachment( &$form, $entityTable, $entityID = null, $numAttachments = null ) {
        ...
        if ( ! empty( $currentAttachments ) ) {
        	$config =& CRM_Core_Config::singleton( );
            foreach ( $currentAttachments as $fileID => $attach ) {
                $time = time() - strtotime($attach['upload_date']);
                if( $attach['mime_type'] === 'image/jpeg') //
                {
                    $currentAttachmentURL[$time] = $attach['fileName']; //$time
