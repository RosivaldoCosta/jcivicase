...
SELECT f.id, f.label, f.data_type,
       ...
       f.date_format,f.time_format,g.extends_entity_column_value 
   ...
   AND f.id IN ( $idString )";
        ...
        while ( $dao->fetch( ) ) {
            ...
            $this->_fields[$dao->id] = array( 'id'              => $dao->id,
                                              ...
                                              'extends_type_id'         => substr($dao->extends_entity_column_value,1,strlen($dao->extends_entity_column_value)-1),
	    ...
	    $this->_activityTypes[] = $dao->extends_entity_column_value;
...
";
    ...
    function select( ) 
    {
        ...
        foreach ( $this->_fields as $id => $field ) {
            ...
            if ( $joinTable ) {
                if ( $joinTable == 'case_activity' ) 
                {
                    $this->_tables["civicrm_contact_type"] = "\nLEFT JOIN civicrm_contact_type ON civicrm_contact_type.name = contact_a.contact_type";
                    $this->_tables["civicrm_case"] = "\nLEFT JOIN civicrm_case ON civicrm_case.case_type_id = civicrm_contact_type.id";
                }
                ...
                if ( $joinTable != 'contact_a' ) {
                    ...
                    if ( $joinTable == 'case_activity' ) 
                    {
                        $this->_whereTables["civicrm_contact_type"] = $this->_tables["civicrm_contact_type"] = 1;
                        $this->_whereTables["civicrm_case"] = $this->_tables["civicrm_case"] = 1;
                    }
                }
            }
        }
