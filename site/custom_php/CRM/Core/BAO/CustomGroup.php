...
class CRM_Core_BAO_CustomGroup extends CRM_Core_DAO_CustomGroup 
{
    ...
    static function create( &$params )
    {
        ...
        if ( $tableName ) {
            $logMessage = 'Custom group "'.$group->title.'" Created';
            CRM_Utils_Hook::post( 'create', 'CustomGroup', $group->id, $group );
        } else {
            $logMessage = 'Custom group "'.$group->title.'" Updated';
...
";
		    //crm_core_error::debug('$queryString', $query );
...
";
                        ...
                        foreach ( $groupTree as $groupID => $group ) {
                            ...
                            foreach ( $group['fields'] as $fieldID => $dontCare ) {
                                ...
                                if ( $dataType == 'File' ) {
                                    ...
                                    $customValue = array( 'id'   => $dao->$idName,
                                                          'data' => html_entity_decode ($dao->$fieldName ));
                                }
    ...
    { 
        ...
        $logMessage = 'Group "' . $group->title . '" Deleted';
        //delete  custom group
        $group->delete();
        $session = & CRM_Core_Session::singleton();
        $id = $session->get('userID');
    ...
    static function setDefaults( &$groupTree, &$defaults, $viewMode = false, $inactiveNeeded = false ) 
    {
        ...
        foreach ( $groupTree as $id => $group ) {
                ...
                case 'Select Date':
                    if ( isset( $value ) ) {
                        if ( !isset($field['time_format'])  ) {
                            ...
                            if (!$value)
                            {
                                list( $defaults[$elementName], $defaults[ $elementName. '_time' ] ) = array('','');
                            }
                            else
                            {
                                list( $defaults[$elementName], $defaults[ $elementName. '_time' ] ) = CRM_Utils_Date::setDateDefaults( $value, null, $field['date_format'], $field['time_format'] );
                            }
                        }
                    }
			...
			/*echo $elementName." = ". $field['id'] . " = " . $value . "<br/>";*/
