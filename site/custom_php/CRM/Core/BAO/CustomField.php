...
class CRM_Core_BAO_CustomField extends CRM_Core_DAO_CustomField 
{
    ...
    static function &dataType()
    {
        if ( !(self::$_dataType) ) {
            self::$_dataType = array(
                                     ...
                                     'CaseReference'	=> ts('Case Reference'),
                                     'Phone'            => ts('Phone'),
                                     );
        }
    ...
    static function create( &$params )
    {
        ...
        if ( CRM_Utils_Array::value( 'id', $params ) ) {
            ...
            $logMessage = 'Custom field "' . $customField->label . '" Updated in "' . CRM_Core_BAO_CustomGroup::getTitle($customField->custom_group_id) . '" group';
        } else {
            ...
            $logMessage = 'Custom field "'.$customField->label.'" created for "' . CRM_Core_BAO_CustomGroup::getTitle($customField->custom_group_id) . '"';
        }
    ...
    public static function getKeyID($key, $all = false) 
    {
		...
		//echo $key;
		//print_r($match);
		//echo CRM_Utils_Array::value(2, $match);
            if ( ! $all ) {
                return $match[1];
            } else {
        ...
        } else if (preg_match('/^custom_(\d+)_?(-?\d+)_(\w+)?$/', $key, $match)) {
		//echo $key;
		//print_r($match);
		//echo CRM_Utils_Array::value(2, $match);
            if ( ! $all ) {
                ...
                return array( $match[1], CRM_Utils_Array::value( 2, $match ) );
            } 
                                                ...
                                                $label = null ) 
    {
	    ...
	    } else if ( strripos($field->column_name, 'phone') !== false) {
		$element =& $qf->add('text',
                                 $elementName,
                                 $label,
                                 array('onkeydown' => "javascript:backspacerDOWN(this,event);",
        ...
        case 'Phone':
            ...
            $qf->addRule($elementName, ts('%1 has a wrong format.  Please use format: (XXX)XXX-XXXX', array(1 => $label)) , 'phone');
            break;
        ...
        case 'Radio':
            ...
            if($field->data_type != 'Boolean') {
	    		...
	    		if ( (strripos($field->column_name,'suicide_ideation') !== false) && ($v === 'Yes'))
			{
                    		$choice[] = $qf->createElement('radio', null, '', $l, (string)$v, array('onclick' => 'javascript:newLethality();'));
			}
			else if ( (strripos($field->column_name,'suicide_threat') !== false) && ($v === 'Yes'))
			{ 
                    		$choice[] = $qf->createElement('radio', null, '', $l, (string)$v, array('onclick' => 'javascript:newLethality();'));
			}
			else
        ...
        case 'Multi-Select Case':
            //Add Multi-select for Linked and Related Cases
    ...
    { 
        ...
        require_once 'CRM/Core/BAO/CustomGroup.php';
        $logMessage = 'Custom field "' . $field->label . '" Deleted from "' . CRM_Core_BAO_CustomGroup::getTitle($field->custom_group_id) . '"';
                                           ...
                                           $contactID = null )
    {
		...
		if( array_key_exists('time_format',$option['attributes']))
		{
                          ...
                          $inline = false ) 
    {
        ...
        foreach ( $params as $key => $value ) 
        {
            if ( $customFieldInfo = CRM_Core_BAO_CustomField::getKeyID( $key, true ) ) 
            {
        		...
        		if (preg_match('/^custom_(\d+)_?(-?\d+)_(\w+)?$/', $key, $match)) 
        		{
				if( array_key_exists('custom_'.$match[1].'_-1', $params) )
				{
					if($params[$key] )
					{
				        	$params[$key] = $params['custom_'.$match[1].'_-1'] .' '.$value;
                            			$params['custom_'.$match[1].'_-1'] .= ' ' . $value;
        				}
        				else
        				{
            					$params[$key] = $params['custom_'.$match[1].'_-1'];
        				}
				}
                   			 $value = $params[$key];
