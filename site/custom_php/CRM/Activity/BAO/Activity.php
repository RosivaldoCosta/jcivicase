...
static function isCurrentRevision($activityId, $context = null)
        {
    jimport('joomla.error.log');
    $log = &JLog::getInstance('iscurrentrevision.log');
    $log->addEntry(array('comment'=> 'Activity ID:' .$activityId));

                $db = &JFactory::getDBO();

                $query = 'SELECT original_id FROM civicrm_activity where id='.$activityId;
                ...
                if(isset($originalId))
                {       
                                $query = 'SELECT id,is_current_revision FROM civicrm_activity where is_current_revision=1 and original_id='.$originalId;
                                $db->setQuery($query);
                                $db->query();

                                if($db->getNumRows() > 0)
                                {
                                        $log->addEntry(array('comment'=> 'Original ID:' .$originalId .' = '.$db->getNumRows()));

          if($context === 'civicrm_caseHistory')
          {
                                    $query = 'SELECT max(id) FROM civicrm_activity where is_current_revision=1 and original_id='.$originalId .' limit 1';
                                    $db->setQuery($query);
                                    $activityId = $db->loadResult();

                                          //$q = "update civicrm_activity set is_current_revision=0 where original_id=".$originalId." and id<>".$max_activity_id;
                                          $log->addEntry(array('comment'=> 'Max ID: ' .$activityId.' Num Rows: '.$db->getNumRows()));
          } 
          
          $q = "update civicrm_activity set is_current_revision=0 where original_id=".$originalId." and id<>".$activityId;
          $db->setQuery($q);
                                        $db->query();
                                }
               }        
        ...
        if ( ! isset( self::$_exportableFields[$name] ) ) {
            ...
            if ( $name == 'Activity' ) {
	            ...
	            $fields = array( 
	                           ...
	                           'case_activity_medium_id'      => array( 'title' => ts('Activity Medium'),         'type' => CRM_Utils_Type::T_STRING ),
...
INNER JOIN  civicrm_option_group grp ON ( grp.id = val.option_group_id AND grp.name = %1 )
    ...
    */
    public function getContactActivitySelector( &$params ) {
        ...
        $params['sort']     = isset($params['sortBy'])?isset($params['sortBy']):'';
        $params['caseId']   = null;
