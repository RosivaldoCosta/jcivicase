...
class CRM_Activity_Form_Activity extends CRM_Contact_Form_Task
{
    ...
    function preProcess( ) 
    {
        ...
        $this->_depart = CRM_Utils_Request::retrieve('depart', 'String', $this );
        $this->assign('depart', $this->_depart);
	...
	*/
	...
	{
	$activities = CRM_Activity_BAO_Activity::getPriorAcitivities($this->_activityId);
	if(isset($activities[$this->_activityId]))
		$defaults['completedby_contact_id'] = $activities[$this->_activityId]['name'];
	...
	$informationOnlyActivityId = CRM_Core_OptionGroup::getValue( 'activity_type',
                                                               'Information Only Call',
                                                               'label' );
	$Police_Community_Education = CRM_Core_OptionGroup::getValue( 'activity_type',
                                                               'Police and Community Education',
                                                               'label' );
	$MCT_Missed_Calls_for_Service = CRM_Core_OptionGroup::getValue( 'activity_type',
                                                               'MCT Missed Calls for Service',
                                                               'label' );

        foreach ( $this->_fields as $field => $values ) {
	      if( $this->_activityTypeId == $informationOnlyActivityId
	       || $this->_activityTypeId == $Police_Community_Education
	       || $this->_activityTypeId == $MCT_Missed_Calls_for_Service)
		{
			if ( $this->_action ) 
            		{
                		if ($field == "status_id")
                		{
                    			$this->_fields[$field]["type"] = "hidden";
                    			$values["type"] = "hidden";
                    			reset($this->_fields[$field]["attributes"]);
   		                 	$first_val = key($this->_fields[$field]["attributes"]);
                    			$this->_fields[$field]["label"] = $first_val;
                    			$this->_fields[$field]["attributes"] = null;
                    			$values["label"] = $first_val;
	...
	*/
      	...
      	$dtElement =& $this->addDateTime( 'activity_date_time', ts('Date'), true, array( 'formatType' => 'activityDateTime') );
  
	//$this->freeze($dtElement);
	//$dtElement[0]->freeze();
	//$dtElement[1]->freeze();
	...
	if( $this->_activityTypeId == $informationOnlyActivityId
           ...
           || $this->_activityTypeId == $MCT_Missed_Calls_for_Service )
	{
        	$hiddenTargetContactField =& $this->add( 'hidden', 'target_contact_id', '718' );
	} else {
	...
	//self::_getDisplayNameById( $this->_sourceContactId );
	/*
	if(isset( $_GET['snippet']))
	{
	if($this->_activityId)
	{
		...
		require_once 'CRM/Contact/BAO/GroupContact.php';
        	$contactGroupList =& CRM_Contact_BAO_GroupContact::getContactGroup( $defaults['source_contact_id'], 'Added', null, false, false, false );
        	$contactGroups = array();
			...
			{
        			$completedbyField =& $this->add( 'text', 'completedby_contact_id', ts('Completed By') );
    ...
    static function formRule( &$fields, &$files, $self ) 
    {
        ...
        if ( ! $self->_single && ! $fields['activity_type_id']) {
            $errors['activity_type_id'] = ts('Follow-up task is a required field');
        }
        foreach($fields as $k => $v)
        {
            ...
            {
                $findex = str_replace('custom_664_', '', $k); 
                if ($v['coupon'] && $fields['custom_1664_'.$findex] == '')
                {
                    $errors['custom_1664_'.$findex] = ts('You must enter the number of coupons.');
                }
            }
        }
        ...
        if ( array_key_exists( 'activity_type_id', $fields ) && 
            ...
            $errors['activity_type_id'] = ts('Follow-up task is required field.');
        }
        //FIX me temp. comment
