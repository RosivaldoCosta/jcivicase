...
class adminModelcoupons extends JModel
{
	...
	function __construct()
	{
		...
		$limit			= $mainframe->getUserStateFromRequest( $context.'coup_limit', 'coup_limit', $mainframe->getCfg('list_limit'), 0);
	...
	{
		$orderby	= $this->_buildContentOrderBy();
		$query = " SELECT #__sv_apptpro2_coupons.*, DATE_FORMAT(expiry_date, '%a %b %e, %Y') as expiry, ".
		" (SELECT count(*) FROM #__sv_apptpro2_requests WHERE coupon_code = #__sv_apptpro2_coupons.coupon_code  ".
		"   AND ( request_status = 'accepted' OR request_status = 'attended' OR request_status = 'completed' )) as current_count ".
		" FROM #__sv_apptpro2_coupons ".$orderby;
		//echo $query;
		//exit;
