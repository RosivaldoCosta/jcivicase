...
class adminModelrequests extends JModel
{
	...
	function __construct()
	{
		...
		$limit			= $mainframe->getUserStateFromRequest( $context.'req_limit', 'req_limit', $mainframe->getCfg('list_limit'), 0);
	...
	function _buildQueryForListScreen()
	{
		...
		if($this->getState('filter_user_search') != ""){
			...
			$filter = $filter."LCASE(#__sv_apptpro2_requests.name) LIKE '%".strtolower($this->getState('filter_user_search') )."%' ";
		}
...
//				}
	...
	function _buildContentOrderBy()
	{
		...
		$filter_order     = $mainframe->getUserStateFromRequest( $context.'req_filter_order',      'req_filter_order', 	  'id_requests' );
		$filter_order_Dir = $mainframe->getUserStateFromRequest( $context.'req_filter_order_Dir',  'req_filter_order_Dir', '' );		
