...
class adminModelbookoffs extends JModel
{
	...
	function __construct()
	{
		...
		$limit			= $mainframe->getUserStateFromRequest( $context.'bo_limit', 'bo_limit', $mainframe->getCfg('list_limit'), 0);
	...
	{
		$orderby	= $this->_buildContentOrderBy();
		$query = "SELECT #__sv_apptpro2_bookoffs.*, #__sv_apptpro2_bookoffs.published, resource_id, #__sv_apptpro2_resources.name, ".
		"DATE_FORMAT(off_date, '%W %M %e, %Y') as off_date_display, off_date, ".
		"#__sv_apptpro2_bookoffs.description, CONCAT(DATE_FORMAT(bookoff_starttime, '%H:%i'), '-', DATE_FORMAT(bookoff_endtime, '%H:%i')) as hours ".
		"FROM #__sv_apptpro2_bookoffs LEFT JOIN #__sv_apptpro2_resources ".
		"ON #__sv_apptpro2_bookoffs.resource_id = #__sv_apptpro2_resources.id_resources ".
		" WHERE #__sv_apptpro2_resources.id_resources = ".$this->getState('filter_bookoffs_resource', 0).
		$orderby;
		//echo $query;
		//exit;
