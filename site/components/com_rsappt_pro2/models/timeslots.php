...
class adminModeltimeslots extends JModel
{
	...
	function __construct()
	{
		...
		$limit			= $mainframe->getUserStateFromRequest( $context.'ts_limit', 'ts_limit', $mainframe->getCfg('list_limit'), 0);
	...
	function _buildQueryForListScreen()
	{
		...
		$query = "SELECT #__sv_apptpro2_timeslots.id_timeslots, day_number, #__sv_apptpro2_timeslots.published, resource_id, ".
		" #__sv_apptpro2_resources.name, ".
		" #__sv_apptpro2_timeslots.start_publishing, #__sv_apptpro2_timeslots.end_publishing, ".
		" TIME_FORMAT(timeslot_starttime,'%H:%i') as timeslot_starttime, ".
		" TIME_FORMAT(timeslot_endtime,'%H:%i') as timeslot_endtime ".
		" FROM #__sv_apptpro2_timeslots LEFT JOIN #__sv_apptpro2_resources ".
		" ON #__sv_apptpro2_timeslots.resource_id = #__sv_apptpro2_resources.id_resources ";
		if($filter != ""){
			$query .= " WHERE ".$filter;
		}
		$query .= $orderby;
