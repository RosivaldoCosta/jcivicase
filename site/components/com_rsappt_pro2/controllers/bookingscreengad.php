...
//				}
		...
		if($paypal_submit == "1" && floatval($grand_total) > 0){
			...
			if($facebook == "Yes"){				
				$this->setRedirect( 'index.php?option=com_rsappt_pro2&view=bookingscreengad&format=fb&Itemid='.$frompage_item.'&task='.$next_view.'&req_id='.$last_id->last_id );
			} else {				
				$this->setRedirect( 'index.php?option=com_rsappt_pro2&view=bookingscreengad&Itemid='.$frompage_item.'&task='.$next_view.'&req_id='.$last_id->last_id );
			}
