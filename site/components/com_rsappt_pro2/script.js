...
function showSlots() {	
	...
	if (xhr.readyState == 4) {
		...
		if (xhr.status == 200) {		
			var outMsg = trim(xhr.responseText);
		} 
		else {
...
function showServices() {	
	...
	if (xhr2.readyState == 4) {
		...
		if (xhr2.status == 200) {		
			var outMsg = trim(xhr2.responseText);
		} 
		else {
...
function trim(inStr){
	return inStr.replace(/^\s+|\s+$/g, '').replace(/\n|\r/g, '') ;
}
...
function showResourceUFDs() {	
	...
	if (xhr3.readyState == 4) {
		...
		if (xhr3.status == 200) {		
			var outMsg = trim(xhr3.responseText);
		} 
		else {
...
function showResourceSeatTypes() {	
	...
	if (xhr4.readyState == 4) {
		...
		if (xhr4.status == 200) {		
			var outMsg = trim(xhr4.responseText);
		} 
		else {
...
function showExtras() {	
	...
	if (xhr5.readyState == 4) {
		...
		if (xhr5.status == 200) {		
			var outMsg = trim(xhr5.responseText);
		} 
		else {
