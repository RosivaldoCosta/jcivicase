...
//						$link 	= 'index.php?option=com_rsappt_pro2&controller=admin_detail&task=edit&cid[]='. $booking->id_requests.'&frompage=frontdesk&Itemid='.$this->Itemid;
						$linkCivi 	= JRoute::_( 'index.php?option=com_civicrm&task=civicrm/contact/view/case&reset=1&action=view&show=1&cid='. $booking->user_id . '&id='. $booking->civicrm_case_id);

						$resursTitle = '';
						if(preg_match('/IHIT Visit/',$booking->resname))
						{
							$resurs = $booking->resname;
							if(preg_match('/^Client\'s initial visit\./',$booking->comment_custom ))
							{
								$resursTitle = '&nbsp;-&nbsp;Initial visit';
							}

						}
						else
						{
							$resurs = 'w/ Dr ' . $booking->resname;
						}
						$title = trim($booking->display_starttime)."-".trim($booking->display_endtime)."&nbsp;\n". stripslashes($booking->name). "&nbsp;" . $resurs . $resursTitle ."&nbsp;\n".$booking->ServiceName;
						$s .= "<br>";
						if($booking->civicrm_case_id)
						{
							$s .= "<a class='crm-summary-link' href=" . $linkCivi . "  title=\"".$title."\">";
						}
						$s .= "<span class='calendar_text_".$booking->request_status."'>". $booking->civicrm_case_id . " | </span>";
						$s .= "<span class='calendar_text_".$booking->request_status."'>". $booking->display_starttime."|".stripslashes($booking->name). ' ' . $resurs . "</span>" ;
	...
	*/
	...
	setlocale(LC_ALL, $lang->getTag());
		...
		for($i=0; $i<$NumDays; $i++){
			...
			if(WINDOWS){
				$s .= "        <td colspan=\"8\" >&nbsp;<a href=".$link.">".$array_daynames[$i]." ".iconv('ISO-8859-2', 'UTF-8',strftime($this->week_view_header_date_format, strtotime(strval($i)." day", $dws)))."</a></td>\n";
			} else {
				$s .= "        <td colspan=\"8\" >&nbsp;<a href=".$link.">".$array_daynames[$i]." ".strftime($this->week_view_header_date_format, strtotime(strval($i)." day", $dws))."</a></td>\n";
			}
			$s .= "      </tr>\n";
...
//					$link 	= 'index.php?option=com_rsappt_pro2&controller=admin_detail&task=edit&cid[]='. $booking->id_requests.'&frompage=frontdesk&Itemid='.$this->Itemid;
					...
					$s .= "  <td width=\"5%\" align=\"left\" ><a href=".$linkCivi.">". $booking->civicrm_case_id ."</a></td>\n";
					...
					$s .= "  <td width=\"25%\" align=\"left\"><a href=\"mailto:".$booking->email."\">".$booking->email."</a></td>\n";
//						$s .= "  <td align=\"center\" width=\"10%\"><span class='color_".$booking->payment_status."'>".translated_status($booking->payment_status)."</span></td>\n";
//					$s .= "  <td width=\"5%\" align=\"center\"><span class='color_".$booking->request_status."'>".translated_status($booking->request_status)."</span></td>\n";
					$s .= "</tr>\n";
					$i2++;
				}
			}
...
//			$link 	= 'index.php?option=com_rsappt_pro2&controller=admin_detail&task=edit&cid[]='. $booking->id_requests.'&frompage=frontdesk&Itemid='.$this->Itemid;
			...
			$s .= "  <td align=\"left\" ><a href=".$linkCivi.">". $booking->civicrm_case_id ."</a></td>\n";
...
//			$s .= "  <td align=\"center\" width=\"10%\"><span class='color_".$booking->request_status."'>".translated_status($booking->request_status)."</span></td>\n";
		...
		if($this->showSeatTotals == "true"){
			...
			$s .= "  <td colspan='4' align='right' style=\"border-bottom:solid 1px\">".JText::_('RS1_TS_TOTAL_SEATS')."&nbsp;</td>\n";
			$s .= "  <td width=\"5%\" align=\"center\" style=\"border-top:solid 1px;border-bottom:solid 1px\"> ".$seat_tally."</td>";
			$s .= "  <td colspan='2' style=\"border-bottom:solid 1px\" >&nbsp;</td>\n";
			$s .= "</tr>\n";
		}
	    $s .= "      </table>\n";
		$s .= "    </tr>\n";
	...
	function getBookings($resAdmin, $month, $year, $startDay="", $NumDays="", $mode="month"){
		...
		$sql = "SELECT #__sv_apptpro2_requests.*, #__sv_apptpro2_resources.resource_admins, #__sv_apptpro2_resources.id_resources as res_id, ".
			...
			"#__sv_apptpro2_udfvalues.udf_value as comment_custom, ";
//		#__sv_apptpro2_udfvalues
			...
			'#__sv_apptpro2_udfvalues ON (#__sv_apptpro2_requests.id_requests = #__sv_apptpro2_udfvalues.request_id) AND #__sv_apptpro2_udfvalues.udf_id = 1)'.
			"WHERE ";
