...
//	include_once( JPATH_SITE."/includes/application.php" );
	...
	if($cId = JRequest::getVar('cid',NULL))
	{
		// Init CiviCRM
		...
		require_once $civicrm_root_api . '/api/v2/Contact.php';

//		$user_civicrm_id = civicrm_uf_match_id_get($user_id);
		$uId = civicrm_uf_id_get($cId);

		//$user =& JFactory::getUser($uId);

		$params = array('contact_id' => $cId);
                $u = &civicrm_contact_get($params);
	...
	$userManager =& JFactory::getUser();
	if($userManager->get('usertype') != 'Super Administrator')
	{
...
//	echo "Version:". substr( $VERSION->getShortVersion(), 0, 3 );
	...
	// id for IHIT Visit resource
	$single_resource_id = "0";
	...
	if(JRequest::getVar('mygridstarttime','')!=""){
	   $gridstarttime = JRequest::getVar('mygridstarttime','');
	}
	if(JRequest::getVar('mygridendtime','')!=""){
	   $gridendtime = JRequest::getVar('mygridendtime','');
	}
...
<script language="JavaScript">
	...
	function initial_visit()
	{
		if(document.getElementById("initialvisit").checked == true){
			document.getElementById("user_field0_value").value="Client's initial visit."+document.getElementById("user_field0_value").value;
		} else {
			var str =  document.getElementById("user_field0_value").value;
			document.getElementById("user_field0_value").value=str.replace("Client's initial visit.", "");
		}
	}
...
<div id="sv_apptpro_request_gad">
  <table width="95%" align="center" border="0" cellspacing="2" cellpadding="2" >
    ...
    <tr>
    <?php if($show_admin){?>
      <td colspan="3" > <h3><?php echo JText::_('RS1_IHIT_INPUT_SCRN_TITLE');?></h3></td>
    ...
    <?php } else { ?>
      <td colspan="4" > <h3><?php echo JText::_('RS1_IHIT_INPUT_SCRN_TITLE');?></h3></td>
    <?php } ?>
    </tr>
    <tr>
      <td></td>
...
<?php } ?>
    <tr>
      <td class="sv_apptpro_request_label"><?php echo JText::_('RS1_INPUT_SCRN_NAME');?></td>
  	  <td colspan="3" valign="top"><input name="name" type="text" id="name" class="sv_apptpro_request_text"
      		size="40" maxlength="50" title="<?php echo JText::_('RS1_INPUT_SCRN_NAME_TOOLTIP');?>" value="<?php echo $u[$cId]['sort_name']; ?>"
	...
	<?php } ?>
    <tr>
      <td class="sv_apptpro_request_label"><?php echo JText::_('RS1_INPUT_SCRN_INITIAL_VISIT');?></td>
      <td colspan="3" valign="top"><input name="initialvisit" type="checkbox" id="initialvisit"
      		 title="<?php echo JText::_('RS1_INPUT_SCRN_INITIAL_VISIT_TOOLTIP');?>" size="40" maxlength="50"
              class="sv_apptpro_request_text" onclick="initial_visit();"></td>
    </tr>
    ...
    <tr id="booking_detail" class="sv_gad_user_selection" style="visibility:hidden; display:none">
      <td class="sv_apptpro_request_label" valign="top"><?php echo JText::_('RS1_GAD_SCRN_DETAIL');?><br/><label class="sv_apptpro_errors" id="selected_resource_wait"></label></td>
<!--    <td colspan="3"><table class="sv_gad_user_selection_table" border="0" width="80%">
          <tr class="sv_gad_user_selection_row">
            <td align="center"><label class="sv_apptpro_selected_resource" id="selected_resource"></label></td>
            <td align="center"><label class="sv_apptpro_selected_resource" id="selected_date"></label></td>
            <td align="center"><label class="sv_apptpro_selected_resource" id="selected_starttime"></label>&nbsp;-&nbsp;
    			<label class="sv_apptpro_selected_resource" id="selected_endtime"></label></td>
            <td align="center"><input type="button" value="Remove" /></td>
          </tr>
          <tr>
            <td>&nbsp;</td>
            <td>&nbsp;</td>
    ...
    <tr>
        <td colspan='4'>
          <table class="sv_gad_container_table" cellpadding="2" cellspacing="0" id="gad_container" style="display:none">
            ...
            <tr>
              <td align="center" colspan="3" width="<?php echo $gridwidth?>"><div id="table_here"></div></td>
            </tr>
            <tr>
              <td>&nbsp;</td>
              <td>&nbsp;</td>
...
?>
  ...
  <input type="hidden" id="gad2" value="<?php echo $apptpro_config->use_gad2 ?>" />
  	...
  	<input type="hidden" name="controller" value="ihit" />
	<input type="hidden" name="id" value="<?php echo $user->id; ?>" />
	<input type="hidden" name="task" value="" />
	<input type="hidden" name="frompage" value="ihit" />
