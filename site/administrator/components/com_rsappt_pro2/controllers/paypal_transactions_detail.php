...
class paypal_transactions_detailController extends JController
{
	...
	*/
	function save()
	{
		...
		$model->checkin();
		$this->setRedirect( 'index.php?option=com_rsappt_pro2&controller=paypal_transactions',$msg );
	}
	...
	*/
	function remove()
	{
		...
		if(!$model->delete($cid)) {
			...
			$this->setRedirect( 'index.php?option=com_rsappt_pro2&controller=paypal_transactions' );
		}
	}
	...
	*/
	function cancel()
	{
		...
		$model = $this->getModel('paypal_transactions_detail');
		$model->checkin();
		if(JRequest::getVar('frompage') == 'requests'){
			$this->setRedirect( 'index.php?option=com_rsappt_pro2&controller=requests');
		} else {
			$this->setRedirect( 'index.php?option=com_rsappt_pro2&controller=paypal_transactions',$msg );
		}
	}	
	...
	*/
	...
	/** function saveorder
	*
	* saveorder of the paypal_transactions items
	* 			
	* @param array cid		- array of id
	...
	*/
	...
	function view_txn()
	{
		...
		$sql = "SELECT id_paypal_transactions FROM #__sv_apptpro2_paypal_transactions WHERE txnid = '". JRequest::getVar('txnid')."'";
		//echo $sql;
		...
		JRequest::setVar( 'layout', 'form'  );
		JRequest::setVar( 'hidemainmenu', 1);
		JRequest::setVar( 'cid', $id_to_view);
		JRequest::setVar( 'frompage', 'requests');
