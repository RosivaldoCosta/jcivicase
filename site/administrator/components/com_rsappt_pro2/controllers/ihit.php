...
class ihitController extends JController
{
	...
	function process_booking_request(){
		...
		$initial = JRequest::getVar('initialvisit', '0');
...
//				}
		...
		$new_case = $this->add_case_activity($cId, $caseId, $startdate, $starttime );
...
//				$sqlCivi = "UPDATE #__sv_apptpro2_requests SET civicrm_activity_id=" . $new_case['result'] . ",civicrm_case_id=" .  $caseId . " WHERE id_requests = ".$last_id->last_id;
		...
		if($paypal_submit == "1" && floatval($grand_total) > 0){
			...
			if($apptpro_config->enable_paypal == "Yes" || $apptpro_config->enable_paypal == "Opt"){
				GoToPayPal($last_id->last_id, $apptpro_config, $grand_total, "ihit", $frompage_item);
			}
...
//	$this->setRedirect( 'index.php?option=com_rsappt_pro2&view=ihit&Itemid='.$frompage_item.'&task='.$next_view.'&req_id='.$last_id->last_id.'&cid='.$cId.'&caseid='.$caseId.'&initialvisit='.$initial );
	...
	function add_case_activity($cId, $caseId, $startdate, $starttime)
	{
	    // Prepare CiviCRM envirement
        $civicrm_root_api = JPATH_BASE . DS . 'administrator/components/com_civicrm/civicrm';
		...
		$initialvisit = JRequest::getVar('initialvisit');
		if($initialvisit) $initialvisit = 1;
		else $initialvisit = 0;


		$params = array(	'case_id'			=> $caseId,
							'activity_type_id'	=> CRM_Core_OptionGroup::getValue( 'activity_type', 'IHIT Visit', 'label' ), // activity type IHIT Visit
							...
							'custom'              => array( array(
                 array( 'value' => $initialvisit,
                        'type'  => 'String',
                        'custom_field_id' => 821,
                        'custom_group_id' => 147,
                        'table_name'      => 'civicrm_value_ihit_visit_details_147',
                        'column_name'     => 'initial_visit_821',
                        'is_multiple'     => 0,
                        'file_id'         => null
                        ) ) )
                        );
