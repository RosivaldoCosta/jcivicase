...
class requests_detailController extends JController
{
	...
	*/
	function save()
	{
		...
		$model->checkin();
		$this->setRedirect( 'index.php?option=com_rsappt_pro2&controller=requests',$msg );
	}
	...
	*/
	function remove()
	{
		...
		if(!$model->delete($cid)) {
			...
			$msg = JText::_( 'COM_RSAPPT_REMOVE_OK' );
	...
	*/
	function cancel()
	{
		...
		$model = $this->getModel('requests_detail');
		$model->checkin();
		if(JRequest::getVar("frompage") == "PayPal"){
			$this->setRedirect( 'index.php?option=com_rsappt_pro2&controller=paypal_transactions');
		} else if(JRequest::getVar("frompage") == "UC"){
			$this->setRedirect( 'index.php?option=com_rsappt_pro2&controller=user_credit_detail&task=edit&cid[]='.JRequest::getVar("frompage_item"));
		} else {
			$this->setRedirect( 'index.php?option=com_rsappt_pro2&controller=requests',$msg );
		}
	}	
