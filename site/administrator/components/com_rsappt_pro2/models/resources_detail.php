...
class resources_detailModelresources_detail extends JModel
{
	...
	function store($data)
	{
		//DEVNOTE: Load table class from com_rsappt_pro2/tables/resources_detail.php	
		$row =& $this->getTable();
		...
		if (!$row->id_resources) {
			$where = 'id_resources = ' . $row->id_resources ;
  ...
  //second parameter: condition
	...
	function delete($cid = array())
	{
		...
		if (count( $cid ))
		{
			...
			$this->_db->setQuery("DELETE FROM #__sv_apptpro2_timeslots WHERE resource_id IN (".$cids.")");
			if (!$this->_db->query()) {
				$this->setError($this->_db->getErrorMsg());
				return false;
			}
			$this->_db->setQuery("DELETE FROM #__sv_apptpro2_bookoffs WHERE resource_id IN (".$cids.")");
			if (!$this->_db->query()) {
				$this->setError($this->_db->getErrorMsg());
				return false;
			}
			$this->_db->setQuery("DELETE FROM #__sv_apptpro2_services WHERE resource_id IN (".$cids.")");
			if (!$this->_db->query()) {
				$this->setError($this->_db->getErrorMsg());
				return false;
			}
