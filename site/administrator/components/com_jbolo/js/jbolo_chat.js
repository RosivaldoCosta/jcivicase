...
function createChatBox(chatboxtitle,minimizeChatBox) {
	...
	if (jQuery("#chatbox_"+chatboxtitle).length > 0) { 
		...
		jQuery('#chatbox_'+chatboxtitle+' .chatboxbottom').css('display','block');  
		// IE positioning hack
	...
	.addClass("chatbox")
	.html('<div class="chatboxhead" onclick="toggleChatBoxGrowth(\''+chatboxtitle+'\')"><div class="chatboxtitle"></div><div class="chatboxoptions"><span class="minimize">_</span><span onclick="closeChatBox(\''+chatboxtitle+'\')" class="minimize">X</span></div><br clear="all"/></div><div class="jb_invite">Add Person to Group Chat:<input class="groupinvite" type="text" /></div><div id='+chatboxtitle+'_thumb class="chatboxistatus"><div class="jfb_customstatus"></div><div class="chatboximgs">'+groupchat+sendfile+history+'<img alter="Clear Conversation" title="Clear Conversation" src="'+jb_abs_link+'components/com_jbolo/img/clear.gif" onClick="javascript:clearChat(\''+chatboxtitle+'\');"/></div></div><div class="chatboxcontent"></div><div class="chatboxoffline">'+jb_offlinemsg+'</div><div class="chatboxinput"><textarea class="chatboxtextarea" onkeydown="javascript:return checkChatBoxInputKey(event,this,\''+chatboxtitle+'\');"></textarea></div><div class=chatboxbottom><img src="'+jb_abs_link+'components/com_jbolo/img/smileys/default/smile.jpg" onClick="javascript:showSmiley(this);" /></div>')
	.appendTo(jQuery( "body" ));
	*/
		jQuery(" <div />" ).attr("id","chatbox_"+chatboxtitle)
	.addClass("chatbox")
	.html('<div class="chatboxhead" onclick="toggleChatBoxGrowth(\''+chatboxtitle+'\')"><div class="chatboxtitle"></div><div class="chatboxoptions"><span class="minimize">-</span>&nbsp; <span onclick="closeChatBox(\''+chatboxtitle+'\')" class="minimize">X</span></div><br clear="all"/></div><div id='+chatboxtitle+'_thumb class="chatboxistatus"><div class="jfb_customstatus"></div><div class="chatboximgs">'+sendfile+history+'<img alter="'+jb_transstring.CLEAR_CONV+'" title="'+jb_transstring.CLEAR_CONV+'" src="'+jb_abs_link+'components/com_jbolo/img/clear.gif" onClick="javascript:clearChat(\''+chatboxtitle+'\');"/></div></div><div class="chatboxcontent"></div><div class="chatboxoffline">'+jb_offlinemsg+'</div><div class="chatboxinput"><textarea class="chatboxtextarea" onkeydown="javascript:return checkChatBoxInputKey(event,this,\''+chatboxtitle+'\');"></textarea></div><div class=chatboxbottom><img src="'+jb_abs_link+'components/com_jbolo/img/smileys/default/smile.jpg" onClick="javascript:showSmiley(this);" /></div>')
	.appendTo(jQuery( "body" ));
	...
	/* Changes for status icons */
			...
			{	
				jQuery("#chatbox_"+chatboxtitle+" .chatboxtitle").html(chattitle.substr(0,25));
				jQuery("#chatbox_"+chatboxtitle+" .chatboxtitle").css({'padding-left':
'16px','width':'178px','height': '13px', 'margin-top':'1px', 'background':'url(components/com_jbolo/img/icon-lightblue.gif) no-repeat 0px 2px'});
			}
			else if(udetails.status == '2')
			{
				jQuery("#chatbox_"+chatboxtitle+" .chatboxtitle").html(chattitle.substr(0,25));
				jQuery("#chatbox_"+chatboxtitle+" .chatboxtitle").css({'padding-left':
'16px','height':'13px','margin-top':'1px', 'width':'178px','background':'url(components/com_jbolo/img/icon-lightblue.gif) no-repeat 0px -24px'});
			}
...
'16px','width':'178px','height': '13px', 'margin-top':'1px', 'background':'url(components/com_jbolo/img/icon-lightblue.gif) no-repeat 0px 2px'});
			...
			else
			{
				if(chattitle.length>25)
					var grptitle = chattitle.substr(0,25)+'...';
				else
					var grptitle = chattitle.substr(0,25);

				var grouptip = '<span class="hasTip" title="'+chattitle+'">'+grptitle+'</span>';
				jQuery("#chatbox_"+chatboxtitle+" .chatboxtitle").html(grouptip);
				jQuery("#chatbox_"+chatboxtitle+" .chatboxtitle").css({'padding-left':
'16px','height':'13px','margin-top':'1px', 'width':'178px','background':'url(components/com_jbolo/img/icon-lightblue.gif) no-repeat 0px 2px'});
			}
			*/
	...
	if (minimizeChatBox == 1) {
		...
		if (minimize == 1) {
			...
			jQuery('#'+chatboxtitle+'_thumb').css('display','none');    
			jQuery('#chatbox_'+chatboxtitle+' .chatboxoffline').css('display','none');
			jQuery('#chatbox_'+chatboxtitle+' .chatboxbottom').css('display','none'); 
			jQuery('#chatbox_'+chatboxtitle+' .jb_invite').css('display','none'); 
			if (jQuery.browser.msie && jQuery.browser.version.substr(0,1)<8) {
...
function showSmiley(selector) {
	...
	jQuery.ajax({
		...
		success : function (data) {
			...
			smilehtml = '<img src="'+jb_abs_link+'components/com_jbolo/img/smileys/default/smile.jpg" onClick="javascript:showSmiley(this);" /><div class=jb_smileybox><table><tr>';
...
function smileyClicked(selector) {
	...
	if(smilebackhtml != null)
	{
		...
		for(var i=0;i<smileyarr.length;i++)
		{
			...
			{
				var textareaval = jQuery(selector).parent().parent().parent().parent().parent().parent().parent().find(".chatboxtextarea").val();
				jQuery(selector).parent().parent().parent().parent().parent().parent().parent().find(".chatboxtextarea").val(textareaval+getdata[0]);
				jQuery(selector).parent().parent().parent().parent().parent().parent().parent().find(".chatboxtextarea").focus();
				break;
			}
		}
		return;
	...
	jQuery.ajax({
		...
		success : function (data) {
			...
			for(var i=0;i<smileyarr.length;i++)
			{
				...
				if(getdata[1]==srcarr[srcarr.length-1])
				{
					var textareaval = jQuery(selector).parent().parent().parent().parent().parent().parent().find(".chatboxtextarea").val();
					jQuery(selector).parent().parent().parent().parent().parent().parent().find(".chatboxtextarea").val(textareaval+getdata[0]);
					jQuery(selector).parent().parent().parent().parent().parent().parent().find(".chatboxtextarea").focus();
					break;
				}
			}
		}
...
function popup(mylink, windowname,size) {
	...
	var windowsize = jQuery.extend({}, {width:500,height:250},{}||size);
...
function chatHeartbeat() { 
	...
	jQuery.ajax({
	  url: jb_abs_link+"index.php?option=com_jbolo&action=chatheartbeat&userno="+jb_usrno+"&lststamp="+lststamp+"&tstamp="+tstamp+"&logtim="+wasted_minutes,
	  cache: false,
	  dataType: "json",
	error: function(){
...
function toggleChatBoxGrowth(chatboxtitle) {
	if (jQuery('#chatbox_'+chatboxtitle+' .chatboxcontent').css('display') == 'none') {  
		...
		jQuery('#chatbox_'+chatboxtitle+' .chatboxbottom').css('display','none');  
		jQuery('#chatbox_'+chatboxtitle+' .jb_invite').css('display','none');    
		jQuery('#chatbox_'+chatboxtitle+' .jb_invite .groupinvite').val('');
		jQuery('#chatbox_'+chatboxtitle+' .chatboxoffline').css('display','none');  
...
function sendChatCallback(msg,sendop,chatboxtitle)
{
				...
				if(last_char=='0')
				{
					...
					jQuery('#chatbox_'+chatboxtitle+' .chatboxtitle').css('background','url("components/com_jbolo/img/icon-lightblue.gif") no-repeat scroll 0 -37px transparent');
				}
				else if(last_char=='2')
				{
					sendop = sendop.substring(0, sendop.length-1);
					...
					jQuery('#chatbox_'+chatboxtitle+' .chatboxtitle').css('background','url("components/com_jbolo/img/icon-lightblue.gif") no-repeat scroll 0 -24px transparent');
				}
				else
				{
					jQuery('#chatbox_'+chatboxtitle+' .chatboxoffline').css('display','none');
					...
					jQuery('#chatbox_'+chatboxtitle+' .chatboxtitle').css('background','url("components/com_jbolo/img/icon-lightblue.gif") no-repeat scroll 0 2px transparent');
				}
...
function chat_status(currentstatus) {
	...
	if(currentstatus==0)
	{
		...
		jQuery('#jbusers li:first').removeClass('img_red img_green').addClass('img_off');
	}
	else if(currentstatus==1)
	{
		insidetext=chat_status_array[1];
		jQuery('#jbusers li:first').removeClass('img_red img_off').addClass('img_green');
	}
	else if(currentstatus==2)
	{
		insidetext=chat_status_array[2];
		jQuery('#jbusers li:first').removeClass('img_off img_green').addClass('img_red');
	}
