...
class CRM_Utils_System_Joomla {
    ...
    static function addHTMLHead( $string = null, $includeAll = false ) {
        ...
        if ( $includeAll ) {
            ...
            $document->addStyleSheet( "{$config->resourceBase}deprecate.css" );
            $document->addStyleSheet( "{$config->resourceBase}civicrm.cdn.css" );
//            $document->addStyleSheet( "{$config->resourceBase}civicrm.css" );

            if ( ! $config->userFrameworkFrontend ) {
                $document->addStyleSheet( "{$config->resourceBase}joomla.css" );
            } else {
                $document->addStyleSheet( "{$config->resourceBase}joomla_frontend.css" );
            }
            if ( isset( $config->customCSSURL ) && ! empty( $config->customCSSURL ) ) {
                $document->addStyleSheet( $config->customCSSURL );
            }

            $document->addStyleSheet( "{$config->resourceBase}extras.css" );

            $document->addScript( "{$config->resourceBase}Common.js" );
