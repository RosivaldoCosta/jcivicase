            ...
            <fieldset title="<?php echo JText::_( 'Alphabetical Index' ); ?>">
                ...
                <div class="helpIndex">
                    <ul class="subext">
                        ...
                        //  if ($helpurl) {
                                echo '<li>';
                                echo JHTML::_('link', $fullhelpurl. $k, $v, array('target' => 'helpFrame'));
                                echo '</li>';
                        /*  } else {
                                echo '<li>';
...
function getHelpTOC( $helpsearch )
{
    ...
    require_once(JPATH_BASE.DS.'modules'.DS.'mod_apmenu'.DS.'helper.php');
    $where = " AND name LIKE \"%".$helpsearch."%\" ";
    $menus = modAPMenuHelper::getMenu('z-adminpraise-menu', $where);

    $toc = array();
    //print_r($menus);
    
//    $toc = _getHelpTOC( $helpsearch );
    
    $toc = getItems($menus, $toc);
    //print_r($toc);
    /*
    foreach ($menus as $key => $item) {
            ...
            $title = $item['name']; // trim( $m[1] );
            $url = strtolower($item['name']);// = $title;
            $url = str_replace(" ","",$url);
            if ($title) {
                    $toc[$url] = $title;
            }

            foreach($item['children'] as $skey => $subitem)
            {
                if( $subitem['type'] === 'seperator')
                    continue;
                $suburl = '.'.strtolower($subitem['name']);
                $suburl = str_replace(" ","",$suburl);
                $toc[$url.$suburl] = $subitem['name'];
            }
    }
    asort( $toc );
    */
...
function getItems($menus, &$toc, $parent = null)
{
    foreach ($menus as $key => $item) 
    {
        if( $item['type'] == 'seperator' || $item['name'] == 'separator')
                continue;
        ...
        if( $item['children'] )
            $toc = array_merge($toc, getItems($item['children'], $toc, $title));
    }
    asort( $toc );
    return $toc;
 ...
 */
function _getHelpTOC( $helpsearch )
{
    global $mainframe;
    ...
    $files = JFolder::files( JPATH_BASE.DS.'help'.DS.$langTag, '\.xml$|\.html$' );
//    require_once(JPATH_BASE.DS.'modules'.DS.'mod_apmenu'.DS.'helper.php');
//    $menus = getMenu('z-adminpraise-menu');
