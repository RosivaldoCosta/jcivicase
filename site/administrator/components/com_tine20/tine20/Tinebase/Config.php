 ...
 * @version     $Id: Config.php 18168 2010-12-29 11:04:55Z p.schuele@metaways.de $
 * 
 */
...
class Tinebase_Config
{
    ...
    public function getConfig($_name, $_applicationId = NULL, $_default = NULL, $_fromFile = TRUE)
    {
        ...
        if (count($records) > 0) {
            ...
            if ($_default !== NULL) {
                Setup_Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . ' Config setting "' . $_name . '" with applicationId "'. $_applicationId .'" not found, using default (' . $_default . ').');
            }
        ...
        //Setup_Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . ' Setting config ' . print_r($result, TRUE));
