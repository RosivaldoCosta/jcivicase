 ...
 * @package     Profiles
 */
class Calendar_Backend_Rest //implements Tinebase_Backend_Interface 
{
    /**
    ...
    {
        $this->_config = isset(Tinebase_Core::getConfig()->rest) ? Tinebase_Core::getConfig()->rest: new Zend_Config(array());
        //$this->_connect();
    }
    ...
    public function get($_id, $_getDeleted = FALSE)
    {
        ...
        $this->_httpClient->setMethod(Zend_Http_Client::GET);
        $response = $this->_httpClient->request();
        $ticket = $this->_rawDataToClient($response->getBody());
    ...
    {
        $tickets = new Tinebase_Record_RecordSet('Profiles_Model_Client');
    ...
    {
        $filter = new Profiles_Model_ClientFilter(array());
    ...
    protected function _rawDataToRecords($_rawData)
    {
        ...
        foreach (explode("\n--\n", $_rawData) as $rawClient) {
            $ticket = $this->_rawDataToClient($rawClient);
            if ($ticket->getId()) {
     ...
     * @return Profiles_Model_Client
     */
    protected function _rawDataToClient($_rawData)
    {
        $dataArray = array();
        ...
        return new Profiles_Model_Client($dataArray);
    }
    ...
    protected function _rawDataToHistory($_rawData) {
        $history = new Tinebase_Record_RecordSet('Profiles_Model_ClientHistoryItem');
        ...
        foreach (explode("\n--\n", $_rawData) as $rawHistory) {
            ...
            $history->addRecord(new Profiles_Model_ClientHistoryItem($dataArray));
        }
        ...
        if ($_filter) {
            ...
            foreach ($_filter->getFilterObjects() as $filterObject) {
                ...
                switch ($field) {
                    ...
                    case 'status':
                        if ($op !== '=') {
                            $idx = array_search($value, Profiles_Model_Client::$status);
                            if ($op == '>') {
                                $status = array_slice(Profiles_Model_Client::$status, 0, $idx+1);
                            } else {
                                $status = array_slice(Profiles_Model_Client::$status, $idx, count(Profiles_Model_Client::$status));
                            }
    ...
    protected function _connect()
    {
        if (! $this->_config->rest || $this->_config->rest->url) {
            throw new Tinebase_Exception_NotFound('Could not connect to rest server: No REST url given!');
        }
        ...
        // login
        $this->_httpClient->setMethod(Zend_Http_Client::GET);
        $this->_httpClient->setUri($this->_config->rest->url);
        
        $loginName = $this->_config->rest->username; //"scheduler_client";//Tinebase_Core::getUser()->accountLoginName;
        if ($this->_config->useCustomCredendials) {
            $this->_httpClient->setAuth('smart_client', '23wesdxc@#WESDXC');
        } else {
