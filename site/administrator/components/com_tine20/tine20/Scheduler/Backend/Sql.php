 ...
 * @version     $Id: Sql.php 18643 2011-01-15 17:28:16Z c.weiss@metaways.de $
 */

/**
 * Native tine 2.0 events sql backend
 *
 * Events consists of the properties of Scheduler_Model_Event except Tags and Notes 
 * which are as always handles by their controllers/backends
 * 
 * 
 * @package Scheduler
 */
class Scheduler_Backend_Sql extends Tinebase_Backend_Sql_Abstract
{
    /**
     ...
     * @var string
     */
    protected $_modelName = 'Scheduler_Model_Appointment';
     ...
     * attendee backend
     * 
     * @var Scheduler_Backend_Sql_Attendee
     */
    ...
    public function __construct ($_dbAdapter = NULL, $_modelName = NULL, $_tableName = NULL, $_tablePrefix = NULL)
    {
        ...
        $this->_attendeeBackend = new Scheduler_Backend_Sql_Attendee($_dbAdapter);
    }
    ...
    public function getByProperty($_value, $_property = 'name', $_getDeleted = FALSE) 
    {
        ...
        $filters = new Scheduler_Model_AppointmentFilter();
    ...
    /**
     * Scheduler optimized search function
     *
    ...
    public function search(Tinebase_Model_Filter_FilterGroup $_filter = NULL, Tinebase_Model_Pagination $_pagination = NULL, $_onlyIds = FALSE, $_getDeleted = FALSE)    
    {
        ...
        if (! $attenderFilter) {
            ...
            if ($containerFilter && $containerFilter instanceof Scheduler_Model_SchedulerFilter) {
                $attenderFilter = $containerFilter->getRelatedAttendeeFilter();
            } else {
                $attenderFilter = new Scheduler_Model_AttenderFilter('attender', 'equals', array(
                   'user_type' => Scheduler_Model_Attender::USERTYPE_USER,
    ...
    protected function _getSelect($_cols = '*', $_getDeleted = FALSE)
    {
        ...
        $select->group($this->_tableName . '.' . 'id_requests');
    ...
    protected function _appendEffectiveGrantCalculationSql($_select, $_attenderFilter = NULL)
    {
        ...
        $attendeeWhere = FALSE;
        if ($_attenderFilter instanceof Scheduler_Model_AttenderFilter) {
        ...
        $_select->joinLeft(
            ...
            /* on     */ $this->_db->quoteIdentifier('attendeeaccounts.contact_id') . ' = ' . $this->_db->quoteIdentifier('attendee.user_id') . 
                            ' AND ' . $this->_db->quoteInto($this->_db->quoteIdentifier('attendee.user_type') . '= ?', Scheduler_Model_Attender::USERTYPE_USER),
            /* select */ array());
    ...
    protected function _getImplicitGrantCondition($_requiredGrant, $_user=NULL)
    {
        ...
        if (in_array($_requiredGrant, array(Tinebase_Model_Grants::GRANT_READ, Tinebase_Model_Grants::GRANT_SYNC, Tinebase_Model_Grants::GRANT_EXPORT, Tinebase_Model_Grants::GRANT_PRIVATE))) {
            $readCond = $this->_db->quoteInto($this->_db->quoteIdentifier('attendee.user_type') . ' = ?', Scheduler_Model_Attender::USERTYPE_USER) . 
    ...
    protected function _rawDataToRecord(array $_rawData) {
        ...
        $this->appendForeignRecordSetToRecord($event, 'attendee', 'id', Scheduler_Backend_Sql_Attendee::FOREIGNKEY_EVENT, $this->_attendeeBackend);
    ...
    protected function _rawDataToRecordSet(array $_rawData)
    {
        ...
        foreach ($_rawData as $rawEvent) {
            $events->addRecord(new Scheduler_Model_Event($rawEvent, true));
        }
        
        $this->appendForeignRecordSetToRecordSet($events, 'attendee', 'id', Scheduler_Backend_Sql_Attendee::FOREIGNKEY_EVENT, $this->_attendeeBackend);
     ...
     *
     * @param Scheduler_Model_Event $_event
     */
     ...
     * 
     * @param Scheduler_Model_Evnet $_event
     *
    protected function _saveAttendee($_event)
    {
        ...
        $currentAttendee = $this->_attendeeBackend->getMultipleByProperty($_event->getId(), Scheduler_Backend_Sql_Attendee::FOREIGNKEY_EVENT);
    ...
    */
     ...
     * @return void
     */
    protected function _setRruleUntil(Scheduler_Model_Event $_event)
    {
        if (empty($_event->rrule)) {
            $_event->rrule_until = NULL;
        } else {
            $rrule = $_event->rrule;
            if (! $_event->rrule instanceof Scheduler_Model_Rrule) {
                $rrule = new Scheduler_Model_Rrule(array());
     ...
     * @return Tinebase_Record_RecordSet
     */
    public function getEventAttendee(Scheduler_Model_Event $_event)
    {
        $attendee = $this->_attendeeBackend->getMultipleByProperty($_event->getId(), Scheduler_Backend_Sql_Attendee::FOREIGNKEY_EVENT);
     ...
     *
     * @param Scheduler_Model_Attender $_attendee
     * @return Scheduler_Model_Attender
     */
    public function createAttendee(Scheduler_Model_Attender $_attendee)
    {
     ...
     * @return Scheduler_Model_Attender
     */
    public function updateAttendee(Scheduler_Model_Attender $_attendee)
    {
