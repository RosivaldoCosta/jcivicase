...
Ext.ensible.cal.EventMappings = {
    ...
    VisitType:    {name: 'VisitType', mapping: 'visittype', type: 'string'},
    Status:    {name: 'Status', mapping: 'status_id', type: 'string'},
    CaseId:    {name: 'CaseId', mapping: 'caseid', type: 'int'},
    ContactId:    {name: 'ContactId', mapping: 'contactid', type: 'int'}
};/**
...
Ext.ensible.cal.EventContextMenu = Ext.extend(Ext.menu.Menu, {
    ...
    /**
     * @cfg {String} viewCaseText
     * The text to display for the 'View Case' option in the menu.
     */
    ...
    buildMenu: function(){
        ...
        Ext.apply(this, {
            items: [{
                text: this.viewCaseText,
                iconCls: 'extensible-cal-icon-cal-show',
                scope: this,
                handler: function(){
                    this.fireEvent('eventview', this, this.rec, this.ctxEl);
                }
            },{
    ...
    initComponent: function(){
      ...
      var statuses = [
       ...
       this.statusField = new Ext.form.ComboBox({
        ...
        var leftFields = [this.titleField, this.dateRangeField,this.visitField,this.statusField], 
            rightFields = [this.notesField, this.locationField];
...
alert('End: '+bounds.end);
        ...
        if(!this.eventMenu){
            this.eventMenu = new Ext.ensible.cal.EventContextMenu({
                listeners: {
                    ...
                    'eventview': this.onViewEvent.createDelegate(this)
    ...
    onViewEvent: function(menu, rec, el){
        
    	console.log(rec);
    	console.log(el);
    	var id = rec.data.CaseId;
    	var cid = rec.data.ContactId;
    	var url='index.php?option=com_civicrm&task=civicrm/contact/view/case&reset=1&id='+id+'&cid='+cid+'&action=view&context=search&selectedChild=case&snippet=2';
    	//window.open(url,'casewin');
    	var casewin = new Ext.Window ({
    		  height: 400, 
    		  width: 400, 
    		  title: 'Case '+id, 
    ...
    getEventBodyMarkup : function(){
        if(!this.eventBodyMarkup){
            this.eventBodyMarkup = [
                 '<tpl if="Status==1">',
                 '<i class="ext-strikedw">{Title}</i>',
                 '</tpl>',
                 '<tpl if="Status==2">',
                 '<i class="ext-strikedw">{Title}</i>',
                 '</tpl>',
                '<tpl if="Status !=2 && Status !=1">',
                '{Title}',
                '</tpl>',
                '<tpl if="_isReminder">',
