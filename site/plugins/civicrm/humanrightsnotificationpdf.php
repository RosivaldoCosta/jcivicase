...
class plgCivicrmHumanrightsnotificationpdf extends JPlugin
{

    function plgCivicrmHumanrightsnotificationpdf(& $subject, $config)
    {
	parent::__construct($subject, $config);

	// load plugin parameters
	$this->_plugin = JPluginHelper::getPlugin( 'civicrm', 'humanrightsnotificationpdf' );
...
//echo '<pre>'; print_r($form); die;
	if (isset($form->_activityTypeId) && $form->_activityTypeId == 85 && $formName == 'CRM_Case_Form_Activity' && (CRM_Utils_Array::value( 'snippet', $_GET ) == 3 ))
	{
	    // PDF view rule
...
//echo '<pre>' . print_r($form->embedAttachmentTags,true) . '</pre>';
	ActivityPDFHelper::buildForm_signature_add('Notification',false,'client',$form, $groupTreeIdx);
	ActivityPDFHelper::buildForm_signature_add('Notification',false,'staff',$form, $groupTreeIdx);
	ActivityPDFHelper::buildForm_signature_add('Notification',false,'witness',$form, $groupTreeIdx);
	ActivityPDFHelper::buildForm_signature_add('Notification',false,'physician',$form, $groupTreeIdx);
