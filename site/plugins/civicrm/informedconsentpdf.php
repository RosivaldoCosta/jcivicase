...
class plgCivicrmInformedConsentpdf extends JPlugin
{
...
//echo '<pre>'; print_r($form); die;
	if (isset($form->_activityTypeId) && $form->_activityTypeId == 82 && $formName == 'CRM_Case_Form_Activity' && (CRM_Utils_Array::value( 'snippet', $_GET ) == 3 ))
	{
	    // PDF view rule
...
//echo '<pre>' . print_r($form->embedAttachmentTags,true) . '</pre>';
	ActivityPDFHelper::buildForm_signature_add('Consent_Statement',false,'client',$form, $groupTreeIdx); // for unknown group to end
	ActivityPDFHelper::buildForm_signature_add('Consent_Statement',false,'staff',$form, $groupTreeIdx);
	ActivityPDFHelper::buildForm_signature_add('Consent_Statement',false,'witness',$form, $groupTreeIdx);
	ActivityPDFHelper::buildForm_signature_add('Consent_Statement',false,'physician',$form, $groupTreeIdx);
