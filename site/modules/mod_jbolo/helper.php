		...
		if ($user->id) { 
			...
			{ //this is for community builder
				...
				else
				{
					...
					WHERE u.id=b.user_id AND u.id IN ( s.userid ) AND s.client_id = 0
					ORDER BY u.".$chattitle ;
				}	
			...
			else if( $chat_config['community']==2 )
			{
				...
				"WHERE u.id = s.userid AND s.client_id = 0
				ORDER BY u.".$chattitle ;
			}
			...
			// this is for jomsocial community 
				...
				else
				{
					...
					. ' WHERE b.id=cu.userid AND b.id IN (s.userid) AND s.client_id = 0'
					. ' ORDER BY b.'.$chattitle ;
				}	
			}
			else
