...
require_once 'CiviTest/Case.php';

define('LETHALITY',35);
define('CAREPLAN',36);
define('ASSESSMENT_TREATMENT',37);
define('MCTDISPATCH',38);
define('CONSENT_FOR_SERVICES',39);
define('AUTHORIZATION_TO_RELEASE',40);
define('DISCHARGE', 47);
define('PHONE_CONTACT', 48);
define('PROGRESSNOTE',50);
define('INDIVIDUAL_TREATMENT',51);
define('INFORMED_CONSENT',82);
define('PRIVACY_PRACTICE',84);
define('HUMAN_RIGHTS',85);
define('PHARMACY_VOUCHER',86);
define('TEMPORARY_HOUSING',87);
define('MOTEL_VOUCHER',88);
define('TRANSPORTATION_VOUCHER',89);
define('MEDEVAL',90);
define('MEDICATIONHISTORY',92);
define('INFORMED_CONSENT_MEDICATION',93);

class CRM_Case_Form_Activity_DischargeTest extends CiviUnitTestCase 
{
    ...
    function get_info( ) 
    {
        return array(
                     ...
                     'description' => 'Test all Case Activity forms.',
                     'group'       => 'CiviCRM Activity Tests',
                     );
    }
     ...
     */
    function testCreateDischarge( )
    {
        $contactId = Contact::createIndividual( );
        $sourceId = Contact::createIndividual( );
        $caseId = Case::createCase(Id );
        
        $params = array( 
                        'source_contact_id'  => $sourceId,
                        'activity_type_id'   => DISCHARGE
                        );
    ...
    /**
     * testcases for createCallActivity() method
     * create() method Add/Edit activity.
     */
    function testCreateCallActivity( )
    {
        $contactId = Contact::createIndividual( );
        ...
        $activityId = $this->assertDBNotNull( 'CRM_Activity_DAO_Activity', 'Incoming Phone Call' , 'id',
                                              'subject', 'Database check for created activity.' );
        ...
        $activityTypeId = $this->assertDBNotNull( 'CRM_Activity_DAO_Activity', 'Incoming Phone Call',
                                                  'activity_type_id',
                                                  'subject', 'Database check on updated activity record.' );
        $this->assertEquals( $activityTypeId, 3, 'Verify activity type id is 3.');
 	...
 	function testGetAllPhoneCallActivities()
	{
        	$params = array(
                        'id'                 => $activityId,
                        'source_contact_id'  => $contactId,
                        'subject'            => 'Incoming Phone Call',
                        'activity_type_id'   => PHONE_CONTACT 
                        );
